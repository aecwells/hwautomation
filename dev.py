...
class TaskRunner:
    ...
    def task_test(self, test_type: str = "all", verbose: bool = False, fail_fast: bool = False):
        """Run tests."""
    ...
    def task_security(self):
        ...
        try:
            ...
            safety_result = run_command(cmd, "Running Safety vulnerability check", check=False)
            result.returncode = max(result.returncode, safety_result.returncode)
        except FileNotFoundError:
            print_colored("💡 Install safety for vulnerability checking: pip install safety", Colors.WARNING)
    ...
    def task_docs(self):
        ...
        if conf_file.exists():
            cmd = [sys.executable, "-m", "sphinx", "-b", "html", str(docs_dir), str(docs_dir / "_build" / "html")]
            run_command(cmd, "Building Sphinx documentation")
        else:
            print_colored("📋 No Sphinx configuration found, generating API docs", Colors.WARNING)
    ...
    def task_dev_server(self, host: str = "127.0.0.1", port: int = 5000):
        ...
        env.update({
            ...
            "FLASK_DEBUG": "1",
            "FLASK_APP": str(app_file)
        })
        
        cmd = [sys.executable, str(app_file)]
        print_colored(f"🌐 Server will be available at http://{host}:{port}", Colors.OKGREEN)
...
def main():
    ...
    parser.add_argument("--fix", action="store_true",
                       help="Automatically fix issues (for lint task)")
    parser.add_argument("--open", action="store_true",
                       help="Open report in browser (for coverage task)")
    ...
    parser.add_argument("--tag", default="hwautomation:latest",
    ...
    task_methods = {
        ...
        "test-integration": lambda: runner.task_test("integration", args.verbose, args.fail_fast),
        "test-performance": lambda: runner.task_test("performance", args.verbose, args.fail_fast),
