...
class MockFirmwareManager:
    async def check_firmware_versions(self, device_type: str, target_ip: str, username: str, password: str) -> Dict[FirmwareType, FirmwareInfo]:
        await asyncio.sleep(0.1)
        if device_type == "a1.c5.large":
            return {
                FirmwareType.BIOS: FirmwareInfo(FirmwareType.BIOS, "U30_v2.50", "U30_v2.54", True, UpdatePriority.HIGH, "/opt/firmware/hpe/bios/U30_v2.54.fwpkg", 480, True),
                FirmwareType.BMC: FirmwareInfo(FirmwareType.BMC, "2.75", "2.78", True, UpdatePriority.CRITICAL, "/opt/firmware/hpe/bmc/ilo5_278.fwpkg", 360, True),
            }
        return {
            FirmwareType.BIOS: FirmwareInfo(FirmwareType.BIOS, "3.4", "3.4", False, UpdatePriority.NORMAL, None, 0, False),
            FirmwareType.BMC: FirmwareInfo(FirmwareType.BMC, "1.73.14", "1.73.14", False, UpdatePriority.NORMAL, None, 0, False),
        }

    async def update_firmware_component(self, firmware_info: FirmwareInfo, target_ip: str, username: str, password: str) -> FirmwareUpdateResult:
        start = datetime.now()
    ...
    async def update_firmware_batch(self, firmware_list: List[FirmwareInfo], target_ip: str, username: str, password: str) -> List[FirmwareUpdateResult]:
        # Simple priority ordering: BMC before BIOS, CRITICAL before others
        order = {UpdatePriority.CRITICAL: 0, UpdatePriority.HIGH: 1, UpdatePriority.NORMAL: 2, UpdatePriority.LOW: 3}
        type_order = {FirmwareType.BMC: 0, FirmwareType.BIOS: 1, FirmwareType.CPLD: 2, FirmwareType.NIC: 3}
        sorted_fw = sorted(firmware_list, key=lambda f: (order[f.priority], type_order.get(f.firmware_type, 99)))
        results: List[FirmwareUpdateResult] = []
        for fw in sorted_fw:
            results.append(await self.update_firmware_component(fw, target_ip, username, password))
        return results
...
async def simulate_firmware_first_provisioning():
    ...
    try:
        ...
        await mon.start_subtask("op", "firmware_analysis", "Analyzing current firmware versions")
        fw_info = await mgr.check_firmware_versions(device_type, target_ip, creds['username'], creds['password'])
        updates_needed = [fw for fw in fw_info.values() if fw.update_required]
        await mon.update_progress("op", 20, f"Found {len(updates_needed)} updates needed")
        await mon.complete_subtask("op", "firmware_analysis", True)

        # Step 3: Firmware updates
        if updates_needed:
            await mon.start_subtask("op", "firmware_updates", f"Updating {len(updates_needed)} firmware components")
            results = await mgr.update_firmware_batch(updates_needed, target_ip, creds['username'], creds['password'])
            successful = [r for r in results if r.success]
            failed = [r for r in results if not r.success]
            await mon.update_progress("op", 50, f"Updates: {len(successful)} successful, {len(failed)} failed")
            ...
            if any(r.requires_reboot for r in successful):
                await mon.start_subtask("op", "system_reboot", "System reboot and validation")
        ...
        await mon.start_subtask("op", "bios_config", "Applying BIOS configuration with monitoring")
        await asyncio.sleep(0.2)
        ...
        await mon.complete_operation("op", True, "Firmware-first provisioning completed successfully")
        return True
