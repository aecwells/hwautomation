...
class ServerResource(BaseResource):
    ...
    def get_single(self, server_id: str):
        ...
        if not self.validate_server_id(server_id):
            return self.error_response(
                "Invalid server ID format", "INVALID_SERVER_ID", 400
            )
        ...
        if cached_server:
            return self.success_response(
                data=cached_server, message=f"Server {server_id} retrieved from cache"
            )
        ...
        try:
            ...
            if not server_data:
                return self.error_response(
                    f"Server {server_id} not found", "SERVER_NOT_FOUND", 404
                )
            ...
            return self.success_response(
                data=server_dict, message=f"Server {server_id} retrieved successfully"
            )

        except Exception as e:
            return self.error_response("Database error occurred", "DATABASE_ERROR", 500)
    ...
    def get_list(self):
        ...
        try:
            ...
            pagination_info = self.paginate_response(
                items=servers, page=page, per_page=per_page, total=total_count
            )

            return self.success_response(
                data=pagination_info, message=f"Retrieved {len(servers)} servers"
            )

        except Exception as e:
            return self.error_response(
                "Failed to retrieve servers", "DATABASE_ERROR", 500
            )
    ...
    def create(self):
        ...
        field_types = {"hostname": str, "ip_address": str, "device_type": str}
        ...
        if not self.validate_ip_address(data["ip_address"]):
            return self.error_response(
                "Invalid IP address format", "INVALID_IP_ADDRESS", 400
            )

        # Validate device type format
        if not self.validate_device_type(data["device_type"]):
            return self.error_response(
                "Invalid device type format", "INVALID_DEVICE_TYPE", 400
            )
        ...
        try:
            ...
            return self.error_response("Failed to create server", "DATABASE_ERROR", 500)
    ...
    def update(self, server_id: str):
        ...
        if not update_fields:
            return self.error_response(
                "No valid fields to update", "NO_UPDATE_FIELDS", 400
            )
        ...
        try:
            ...
            return self.error_response("Failed to update server", "DATABASE_ERROR", 500)
    ...
    def delete_resource(self, server_id: str):
        ...
        try:
            ...
            return self.success_response(
                message=f"Server {server_id} deleted successfully", status_code=204
            )

        except Exception as e:
            return self.error_response("Failed to delete server", "DATABASE_ERROR", 500)
...
class EnhancedServerAPI:
    ...
    def __init__(self, app, socketio=None):
        ...
        if socketio:
            ...
            self.server_status_manager = self.websocket_factory.create_manager(
                "server_status"
            )
    ...
    def create_server_enhanced(self, json_data):
        ...
        try:
            ...
            return self.response_handler.internal_error("Failed to create server")
