...
def main():
    parser = argparse.ArgumentParser(description="Test vendor-specific hardware discovery")
    ...
    parser.add_argument('--manufacturer', help='Force specific manufacturer detection (supermicro, hpe, dell)')
    ...
    try:
        ...
        if result.success:
            ...
            if 'supermicro' in manufacturer:
                print("🔧 Supermicro Device Detected - Testing sumtool functionality...")
                test_supermicro_tools(discovery_manager, args.host, args.user, args.ssh_key)
                ...
                print("🔧 Dell Device Detected - Testing Dell OpenManage functionality...")
    ...
    try:
        with ssh_manager.get_client(hostname, username, ssh_key_path) as ssh_client:
            ...
            if result.exit_code == 0:
                ...
                print("    ⚠️  sumtool not available - would be installed during discovery")
    ...
    try:
        with ssh_manager.get_client(hostname, username, ssh_key_path) as ssh_client:
            ...
            if available_tool:
                ...
                result = ssh_client.execute_command(f"sudo {available_tool} ctrl all show config")
                if result.exit_code == 0:
                    print("    ✅ Controller Information - Command executed successfully")
                    ...
                    print(f"    ⚠️  Controller query failed (may be normal if no controllers present)")
            else:
                print("    ⚠️  HPE tools not available - would be installed during discovery")
    ...
    try:
        with ssh_manager.get_client(hostname, username, ssh_key_path) as ssh_client:
            ...
            if result.exit_code == 0:
                ...
                print("    ⚠️  Dell OpenManage tools not available - would be installed during discovery")
