...
def main():
    parser = argparse.ArgumentParser(
        description="Enhanced server commissioning with database integration"
    )
    parser.add_argument("--server-id", required=True, help="Server ID in MaaS")
    parser.add_argument(
        "--device-type", required=True, help="Device type (e.g., s2.c2.small)"
    )
    ...
    try:
        ...
        if result["success"]:
            ...
            print(
                f"  SSH Verified: {context.get('ssh_connectivity_verified', 'Unknown')}"
            )
...
def show_workflow_plan(args):
    ...
    steps = [
        (
            "1. Commission Server",
            f"Commission {args.server_id} in MaaS and create database entry",
        ),
        ("2. Get Server IP", f"Retrieve IP address and test SSH connectivity"),
        (
            "3. Hardware Discovery",
            f"Discover hardware info including IPMI, store in database",
        ),
        ...
        (
            "8. Finalize Server",
            f"Tag server and update database with completion status",
        ),
    ]
...
def show_database_state(db_helper: DbHelper, server_id: str):
    ...
    try:
        ...
        cursor.execute(
            f"SELECT * FROM {db_helper.tablename} WHERE server_id = ?", (server_id,)
        )
