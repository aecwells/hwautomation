...
def example_workflow():
    ...
    for machine in machines:
        ...
        if ip_address:
            ...
            print(f"Machine {system_id}: IP {ip_address} ({'reachable' if is_reachable else 'unreachable'})")
    ...
    if ipmi_ips:
        ...
        if 'password' in config['ipmi']:
            ...
            for ipmi_ip in ipmi_ips[:1]:  # Check first one only
                power_status = ipmi_manager.get_power_status(ipmi_ip, config['ipmi']['password'])
...
def example_redfish_operations():
    ...
    if server_ips:
        ipmi_ips = ipmi_manager.get_ipmi_ips_from_servers(server_ips[:1])  # Get one for demo
...
def example_database_migration():
    ...
    print("You can examine it with: python scripts/db_manager.py status -d migration_test.db")
