...
def discover_system_info(target_ip: str, username: str, password: str):
    ...
    try:
        with RedfishManager(target_ip, username, password) as redfish:
            ...
            print(f"\nCapabilities:")
            print(f"  System Info: {'✅' if capabilities.supports_system_info else '❌'}")
            print(f"  Power Control: {'✅' if capabilities.supports_power_control else '❌'}")
            print(f"  BIOS Config: {'✅' if capabilities.supports_bios_config else '❌'}")
    ...
    try:
        with RedfishManager(target_ip, username, password) as redfish:
            ...
            if bios_settings:
                ...
                print("   Use --modify-bios flag to test setting changes (with caution!)")
...
def compare_with_bios_manager(target_ip: str, username: str, password: str):
    ...
    try:
        ...
        redfish_success, redfish_msg = bios_manager.test_redfish_connection(target_ip, username, password)
        print(f"Redfish via BiosConfigManager: {'✅' if redfish_success else '❌'} {redfish_msg}")
        
        # Test method determination
        device_type = "a1.c5.large"  # Example device type
        method = bios_manager.determine_bios_config_method(target_ip, device_type, username, password)
        print(f"Recommended BIOS config method for {device_type}: {method}")
        
        # Get system info via BIOS manager
        system_info = bios_manager.get_system_info_via_redfish(target_ip, username, password)
        if system_info:
            print(f"System info via BiosConfigManager: {system_info.manufacturer} {system_info.model}")
        else:
...
def test_enhanced_bios_config(target_ip: str, username: str, password: str, device_type: str, dry_run: bool = True):
...
def main():
    ...
    parser.add_argument("--username", default="ADMIN", help="BMC username (default: ADMIN)")
    parser.add_argument("--password", required=True, help="BMC password")
    parser.add_argument("--device-type", default="a1.c5.large", help="Device type for BIOS config test")
    parser.add_argument("--modify-bios", action="store_true", help="Actually modify BIOS settings (use with caution!)")
    parser.add_argument("--power-control", help="Power control action (On, ForceOff, etc.)")
    ...
    if args.power_control:
        ...
        try:
            with RedfishManager(args.target, args.username, args.password) as redfish:
                ...
                print(f"Power control result: {'✅ Success' if success else '❌ Failed'}")
        except Exception as e:
    ...
    test_enhanced_bios_config(args.target, args.username, args.password, args.device_type, dry_run)
