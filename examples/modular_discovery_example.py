...
class ModularHardwareDiscoveryManager:
    ...
    def discover_hardware(
        self, hostname: str, username: str = "ubuntu", ssh_key_path: str = None
    ) -> HardwareDiscovery:
        """
        ...
        try:
            ...
            ssh_client = self.ssh_manager.get_client(
                hostname=hostname, username=username, key_filename=ssh_key_path
            )
            ...
            if vendor_discovery:
                ...
                if not self._ensure_vendor_tools(ssh_client, vendor_discovery):
                    errors.append(
                        f"Failed to install {vendor_discovery.vendor_name} tools"
                    )
                else:
    ...
    def _ensure_vendor_tools(
        self, ssh_client: SSHClient, vendor_discovery: BaseVendorDiscovery
    ) -> bool:
