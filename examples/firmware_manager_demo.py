...
"""
        ...
        try:
            ...
            for fw_type, fw_info in firmware_info.items():
                ...
                priority = (
                    f"(priority: {fw_info.priority.value})"
            ...
            if updates_needed:
                ...
                print(
                    f"   Estimated time: {total_time} seconds ({total_time/60:.1f} minutes)"
                )
                ...
                for result in results:
                    ...
                    print(
                        f"     {status} {result.firmware_type.value}: {result.old_version} → {result.new_version}"
                    )
                    print(
                        f"        Time: {result.execution_time:.1f}s, Reboot: {result.requires_reboot}"
                    )
