...
def discover_hardware(host: str, username: str = 'ubuntu', key_file: str = None, 
...
def scan_network(network_range: str, username: str = 'ubuntu', key_file: str = None, 
                verbose: bool = False):
    """
    ...
    try:
        ...
        ipmi_addresses = discovery_manager.discover_ipmi_from_network_scan(network_range)
...
Examples:
  ...
  %(prog)s scan 192.168.1.0/24
    ...
    # Discover command
    discover_parser = subparsers.add_parser('discover', help='Discover hardware from single host')
    discover_parser.add_argument('host', help='Target hostname or IP address')
    discover_parser.add_argument('-u', '--username', default='ubuntu', 
                               help='SSH username (default: ubuntu)')
    discover_parser.add_argument('-k', '--key-file', help='SSH private key file path')
    discover_parser.add_argument('-f', '--format', choices=['json', 'yaml', 'text'], 
                               default='json', help='Output format (default: json)')
    discover_parser.add_argument('-v', '--verbose', action='store_true', 
                               help='Enable verbose logging')
    
    # Scan command
    scan_parser = subparsers.add_parser('scan', help='Scan network range for IPMI addresses')
    scan_parser.add_argument('network', help='Network range to scan (e.g., 192.168.1.0/24)')
    scan_parser.add_argument('-u', '--username', default='ubuntu',
                           help='SSH username (default: ubuntu)')
