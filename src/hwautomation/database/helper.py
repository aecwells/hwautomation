...
class DbHelper:
    ...
    def __init__(self, tablename: str, db_path: str = ":memory:", auto_migrate: bool = True):
        """
    ...
    def createdbtable_legacy(self):
        ...
        self.sql_db_worker.execute(" CREATE TABLE IF NOT EXISTS " + self.tablename + " ("
    ...
    def createdbtable(self):
        ...
        try:
            ...
            self.sql_db_worker.execute(f"SELECT name FROM sqlite_master WHERE type='table' AND name='{self.tablename}'")
    ...
    def createrowforserver(self, serverid: str):
        ...
        self.sql_db_worker.execute(
            f"INSERT INTO {table_name} (server_id) VALUES (?)",
            (serverid,)
        )
    ...
    def checkifserveridexists(self, serverid: str) -> List[bool]:
        ...
        table_name = self._get_table_name()
        check = [item[0] for item in self.sql_db_worker.execute(
    ...
    def checkready(self, serverid: str):
        ...
        return self.sql_db_worker.execute(
            f"SELECT is_ready FROM {table_name} WHERE server_id = ?",
            (serverid,)
        )
    ...
    def update_server_metadata(self, server_id: str, **kwargs):
        ...
        if valid_updates:
            ...
            self.sql_db_worker.execute(
                f"UPDATE {table_name} SET {set_clause} WHERE server_id = ?",
                values
            )
            self.sql_db_worker.commit()
    ...
    def get_server_by_id(self, server_id: str) -> Optional[Dict]:
        ...
        table_name = self._get_table_name()
        result = self.sql_db_worker.execute(
            f"SELECT * FROM {table_name} WHERE server_id = ?",
            (server_id,)
        ).fetchone()
