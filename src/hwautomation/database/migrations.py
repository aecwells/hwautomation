...
class DatabaseMigrator:
    ...
    def get_all_migrations(self) -> List[tuple]:
        ...
        return [
            ...
            (6, "Add device type and workflow fields", self._migration_006_add_device_workflow_fields),
        ]
    ...
    def _migration_002_add_ipmi_fields(self, cursor):
        ...
        if 'ipmi_username' not in columns:
            cursor.execute("ALTER TABLE servers ADD COLUMN ipmi_username TEXT DEFAULT 'ADMIN'")
        
        if 'ipmi_password_set' not in columns:
            cursor.execute("ALTER TABLE servers ADD COLUMN ipmi_password_set TEXT DEFAULT 'FALSE'")
        
        if 'bios_password_set' not in columns:
            cursor.execute("ALTER TABLE servers ADD COLUMN bios_password_set TEXT DEFAULT 'FALSE'")
        
        if 'redfish_available' not in columns:
            cursor.execute("ALTER TABLE servers ADD COLUMN redfish_available TEXT DEFAULT 'UNKNOWN'")
    ...
    def _migration_003_add_timestamps(self, cursor):
        ...
        if 'created_at' not in columns:
            cursor.execute("ALTER TABLE servers ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP")
        
        if 'updated_at' not in columns:
            cursor.execute("ALTER TABLE servers ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP")
    ...
    def _migration_004_add_metadata(self, cursor):
        ...
        for column_name, column_type in new_columns:
            if column_name not in columns:
                cursor.execute(f"ALTER TABLE servers ADD COLUMN {column_name} {column_type}")
    ...
    def _migration_005_add_power_state(self, cursor):
        ...
        if 'power_state' not in columns:
            cursor.execute("ALTER TABLE servers ADD COLUMN power_state TEXT DEFAULT 'UNKNOWN'")
    ...
    def _migration_006_add_device_workflow_fields(self, cursor):
        ...
        workflow_fields = [
            ('device_type', 'TEXT'),           # Device type for BIOS config (e.g., PowerEdge R7525)
            ('server_type', 'TEXT'),           # Server type category (e.g., compute, storage, etc.)
            ...
            ('hardware_validated', 'INTEGER DEFAULT 0'), # Boolean: Hardware discovery completed (0/1)
