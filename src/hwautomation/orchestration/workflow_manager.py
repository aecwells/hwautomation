...
class WorkflowManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.maas_client = MaasClient(
            ...
            consumer_token=maas_config.get(
                "token_key", ""
    ...
    async def _execute_firmware_first_provisioning(
        ...
        try:
            ...
            if result.success:
                ...
                logger.info(
                    f"  - Firmware updates applied: {result.firmware_updates_applied}"
                )
                logger.info(
                    f"  - BIOS settings applied: {result.bios_settings_applied}"
                )
                ...
                logger.error(
                    f"Firmware-first provisioning failed for {provisioning_context.server_id}: {result.error_message}"
                )
                return False

        except Exception as e:
            workflow_context.report_sub_task(
                f"Firmware-first provisioning error: {str(e)}"
            )
            logger.error(
                f"Firmware-first provisioning exception for {provisioning_context.server_id}: {e}"
            )
            return False
