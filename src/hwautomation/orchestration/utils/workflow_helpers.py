...
class WorkflowHelpers:
    ...
    @staticmethod
    def retry_operation(
        operation, max_retries: int = 3, delay: float = 1.0, backoff_factor: float = 2.0
    ) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.debug(
                        f"Retry attempt {attempt + 1} failed, retrying in {sleep_time}s: {e}"
                    )

        return {"success": False, "error": str(last_exception), "attempts": max_retries}
    ...
    def filter_sensitive_data(
        ...
        for key, value in data.items():
            if any(sensitive_key in key.lower() for sensitive_key in sensitive_keys):
                ...
                filtered[key] = WorkflowHelpers.filter_sensitive_data(
