...
class SSHOperations:
    ...
    def test_connectivity(self, timeout: int = 30) -> Dict[str, Any]:
        ...
        try:
            result = self.ssh_client.execute_command(
                "echo 'connectivity_test'", timeout=timeout
            )
            return {
    ...
    def install_packages(
        self, packages: List[str], update_first: bool = True
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "packages_installed": packages, "details": results}

        except Exception as e:
            return {"success": False, "error": f"Package installation failed: {e}"}

    def check_service_status(self, service_name: str) -> Dict[str, Any]:
        """Check the status of a system service."""
        try:
            # Check if service exists
            exists_result = self.ssh_client.execute_command(
            ...
            active_result = self.ssh_client.execute_command(
                f"systemctl is-active {service_name}"
            )
            is_active = active_result.get(
                "success", False
            ) and "active" in active_result.get("stdout", "")

            # Check if service is enabled
            enabled_result = self.ssh_client.execute_command(
                f"systemctl is-enabled {service_name}"
            )
            is_enabled = enabled_result.get(
                "success", False
            ) and "enabled" in enabled_result.get("stdout", "")
            ...
            return {"exists": False, "active": False, "enabled": False, "error": str(e)}
    ...
    def manage_service(self, service_name: str, action: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Service management failed: {e}"}
    ...
    def transfer_file(
        ...
        try:
            ...
            if create_dirs:
                ...
                if remote_dir:
                    mkdir_result = self.ssh_client.execute_command(
            ...
            if transfer_result.get("success"):
                ...
                verify_result = self.ssh_client.execute_command(
            ...
            return {"success": False, "error": f"File transfer failed: {e}"}
    ...
    def gather_system_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # CPU information
            cpu_result = self.ssh_client.execute_command(
            ...
            # Memory information
            memory_result = self.ssh_client.execute_command(
    ...
    def execute_commands_batch(
        ...
        try:
            ...
            return {"success": False, "error": f"Batch command execution failed: {e}"}
    ...
    def wait_for_condition(
        ...
        try:
            ...
            return {"success": False, "error": f"Condition waiting failed: {e}"}
        ...
        try:
            ...
            for tool_name, command in tools.items():
                ...
                if tools_status[tool_name]["available"]:
                    version_result = self.ssh_client.execute_command(
                        f"{command} --version 2>/dev/null || {command} -V 2>/dev/null || echo 'version unknown'"
                    )
                    if version_result.get("success"):
                        tools_status[tool_name]["version"] = version_result.get(
                            "stdout", ""
                        ).strip()
            ...
            return {"success": False, "error": f"Hardware tools check failed: {e}"}
