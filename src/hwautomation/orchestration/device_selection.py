...
class DeviceSelectionService:
    ...
    def list_available_machines(
        self, machine_filter: MachineFilter = None
    ) -> List[Dict]:
        """
    ...
    def validate_machine_for_commissioning(self, system_id: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if status not in commissionable_statuses:
                return (
                    False,
                    f"Machine is in '{status}' status, not available for commissioning",
                )

            # Check if machine has an owner (shouldn't commission owned machines)
            if (
                machine.get("owner")
                and machine.get("owner")
                and machine["owner"].get("username")
            ):
            ...
            if not machine.get("power_type") or machine.get("power_type") == "manual":
                logger.warning(
                    f"Machine {system_id} has manual power type - may require manual intervention"
                )
    ...
    def _apply_filters(
        ...
        for machine in machines:
            ...
            if machine_filter.min_memory_gb:
                memory_gb = (
                    machine.get("memory", 0) / 1024
                    if machine.get("memory", 0) > 0
                    else 0
                )
            ...
            if machine_filter.min_storage_gb:
                storage_gb = (
                    machine.get("storage", 0) / (1024**3)
                    if machine.get("storage", 0) > 0
                    else 0
                )
            ...
            if machine_filter.architecture:
                if (
                    machine.get("architecture", "").lower()
                    != machine_filter.architecture.lower()
                ):
                    continue
            ...
            if machine_filter.power_type:
                if (
                    machine.get("power_type", "").lower()
                    != machine_filter.power_type.lower()
                ):
                    continue
