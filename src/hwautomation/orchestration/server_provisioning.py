...
class ServerProvisioningWorkflow:
        ...
        try:
            ...
            if context.db_helper:
                if not context.db_helper.checkifserveridexists(context.server_id)[0]:
                    ...
                    logger.info(f"Created database entry for server {context.server_id}")
                
                # Update status to indicate commissioning started and record workflow_id
                context.db_helper.updateserverinfo(context.server_id, 'status_name', 'Commissioning')
                context.db_helper.updateserverinfo(context.server_id, 'is_ready', 'FALSE')
                context.db_helper.updateserverinfo(context.server_id, 'device_type', context.device_type)
                # Update workflow_id if available (get from workflow context/manager)
                if hasattr(context, 'workflow_id') and context.workflow_id:
                    context.db_helper.updateserverinfo(context.server_id, 'workflow_id', context.workflow_id)
                    logger.info(f"Updated database with workflow_id: {context.workflow_id} for server: {context.server_id}")
            ...
            if current_status in ['Ready', 'Commissioned']:
                ...
                if not machine_ip:
                    logger.info(f"Server {context.server_id} is {current_status} but has no SSH-accessible IP address - forcing recommission")
                    ...
                    if not self._test_ssh_connectivity(machine_ip, context):
                        ...
                        if context.db_helper:
                            ...
                            context.db_helper.updateserverinfo(context.server_id, 'is_ready', 'TRUE')
                            context.db_helper.updateserverinfo(context.server_id, 'ip_address', machine_ip)
                            context.db_helper.updateserverinfo(context.server_id, 'ip_address_works', 'TRUE')
                        
                        return {'status': 'commissioned', 'machine_info': server_info, 'ip_address': machine_ip}
            ...
            # Check for other problematic states that require force recommissioning
                ...
                logger.info(f"Server {context.server_id} is in failed state ({current_status}) - forcing recommission")
                should_force_commission = True
            ...
            if context.db_helper and server_info:
                ...
                context.db_helper.updateserverinfo(context.server_id, 'server_model', server_model)
            ...
            while time.time() < timeout:
                ...
                if status in ['Commissioned', 'Ready']:
                    ...
                    if context.db_helper:
                        context.db_helper.updateserverinfo(context.server_id, 'status_name', 'Failed commissioning')
                        context.db_helper.updateserverinfo(context.server_id, 'is_ready', 'FALSE')
                    raise CommissioningError(f"Commissioning failed for {context.server_id}")
                elif status == 'Commissioning':
                    logger.info(f"Server {context.server_id} still commissioning...")
                else:
                    logger.info(f"Server {context.server_id} status: {status} - waiting for completion...")
                
                time.sleep(30)  # Check every 30 seconds
            
            # Commissioning timeout
            if context.db_helper:
                context.db_helper.updateserverinfo(context.server_id, 'status_name', 'Commissioning timeout')
                context.db_helper.updateserverinfo(context.server_id, 'is_ready', 'FALSE')
            ...
            if context.db_helper:
                context.db_helper.updateserverinfo(context.server_id, 'status_name', f'Error: {str(e)}')
                context.db_helper.updateserverinfo(context.server_id, 'is_ready', 'FALSE')
    ...
    def _get_server_ip(self, context: WorkflowContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if not ip_address:
                ...
                if not machine_info:
                    raise Exception(f"Could not retrieve machine information for server {context.server_id}")
            ...
            if context.db_helper:
                context.db_helper.updateserverinfo(context.server_id, 'ip_address', ip_address)
            ...
            if context.db_helper:
                context.db_helper.updateserverinfo(context.server_id, 'ip_address_works', 'TRUE' if ssh_works else 'FALSE')
            
            if not ssh_works:
                raise SSHConnectionError(f"SSH connectivity test failed for {ip_address}")
            ...
            if context.db_helper:
                context.db_helper.updateserverinfo(context.server_id, 'ip_address_works', 'FALSE')
            raise

    def _test_ssh_connectivity(self, ip_address: str, context: WorkflowContext, max_retries: int = 3, timeout: int = 15) -> bool:
        """
    ...
    def _discover_hardware(self, context: WorkflowContext) -> Dict[str, Any]:
        ...
        try:
            ...
            context.hardware_discovery_result = {
                ...
                'network_interfaces': [iface.__dict__ for iface in hardware_result.network_interfaces],
            ...
            if context.db_helper:
                ...
                if hardware_result.system_info.manufacturer:
                    context.db_helper.updateserverinfo(context.server_id, 'server_model', 
                ...
                if hardware_result.ipmi_info and hardware_result.ipmi_info.ip_address:
                    ...
                    if hardware_result.ipmi_info.enabled:
                        context.db_helper.updateserverinfo(context.server_id, 'kcs_status', 'Enabled')
                        context.db_helper.updateserverinfo(context.server_id, 'host_interface_status', 'Active')
            ...
            if context.db_helper:
                context.db_helper.updateserverinfo(context.server_id, 'ipmi_address_works', 'FALSE')
                context.db_helper.updateserverinfo(context.server_id, 'kcs_status', 'Discovery Failed')
            raise
    ...
    def _test_ipmi_connectivity(self, ipmi_ip: str, context: WorkflowContext) -> bool:
        ...
        try:
            ...
            import subprocess
            result = subprocess.run(['ping', '-c', '3', '-W', '5', ipmi_ip], 
            ...
            return {
                ...
                'network_interfaces': [iface.name for iface in hardware_info.network_interfaces]
            }
    ...
    def _detect_server_vendor(self, ssh_client) -> str:
        ...
        try:
            # Try dmidecode first
            stdout, stderr, exit_code = ssh_client.exec_command("sudo dmidecode -s system-manufacturer")
            ...
            stdout, stderr, exit_code = ssh_client.exec_command("ls /sys/class/dmi/id/sys_vendor")
            if exit_code == 0:
                stdout, stderr, exit_code = ssh_client.exec_command("cat /sys/class/dmi/id/sys_vendor")
                if exit_code == 0:
                    vendor = stdout.strip().lower()
                    if 'supermicro' in vendor:
                        return 'Supermicro'
    ...
    def _pull_bios_config_supermicro(self, ssh_client, context: WorkflowContext) -> Dict[str, Any]:
        ...
        logger.debug(f"sumtool output - stdout: '{stdout}', stderr: '{stderr}', exit_code: {exit_code}")
        ...
        if exit_code != 0:
            ...
            help_stdout, help_stderr, help_exit = ssh_client.exec_command("sumtool -h 2>&1")
    ...
    def _create_dummy_bios_config(self, context: WorkflowContext, vendor: str) -> Dict[str, Any]:
        ...
        return {'config_path': local_config_path, 'config_data': context.original_bios_config}
    ...
    def _install_sumtool_on_server(self, ssh_client) -> bool:
        ...
        try:
            ...
            stdout, stderr, exit_code = ssh_client.exec_command("sudo apt-get install -y tar gzip")
            if exit_code != 0:
                logger.error("Failed to install dependencies for sumtool")
                return False
            
            logger.info("Uploading and installing sumtool from local tools directory...")
            ...
            # Verify upload
            stdout, stderr, exit_code = ssh_client.exec_command("ls -la /tmp/sumtool.tar.gz")
            if exit_code != 0:
            ...
            stdout, stderr, exit_code = ssh_client.exec_command("cd /tmp && tar -tzf sumtool.tar.gz | head -5")
            if exit_code != 0:
            ...
            if exit_code == 0:
                logger.info(f"sumtool installed and verified successfully: {stdout.strip()}")
                return True
            else:
                ...
                if exit_code == 0:
                    logger.info("sumtool installed successfully (verified with -h flag)")
                    return True
            ...
            for cmd in fallback_commands:
                ...
                if "ipmitool" in cmd and exit_code == 0:
                    logger.info("Installed ipmitool as fallback - some BIOS operations may be limited")
                    return True
    ...
    def _modify_bios_config(self, context: WorkflowContext) -> Dict[str, Any]:
        """Step 5: Modify BIOS configuration"""
        logger.info(f"Modifying BIOS configuration for device type {context.device_type}")
        
        try:
            # Check if this is a supported vendor
            if hasattr(context, 'original_bios_config') and isinstance(context.original_bios_config, dict):
                if context.original_bios_config.get('supported') == False:
                    vendor = context.original_bios_config.get('vendor', 'Unknown')
                    logger.info(f"Skipping BIOS modification for {vendor} server - not yet supported")
                    ...
                    return {
                        ...
                        'changes_applied': [f'No changes applied - {vendor} BIOS configuration not yet supported'],
                        'vendor_supported': False
                    }
            
            # Load device-specific template
            template = self.manager.bios_manager.get_device_template(context.device_type)
    ...
    def _push_bios_config(self, context: WorkflowContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if vendor == 'supermicro':
                ...
                logger.warning(f"BIOS configuration push not supported for vendor: {vendor}")
                ssh_client.close()
                return {
                    'status': 'skipped',
    ...
    def _configure_ipmi_conditional(self, context: WorkflowContext) -> Dict[str, Any]:
        ...
        if not context.target_ipmi_ip:
            ...
            if context.db_helper:
                context.db_helper.updateserverinfo(context.server_id, 'kcs_status', 'Configuration Skipped')
    ...
    def _configure_ipmi(self, context: WorkflowContext) -> Dict[str, Any]:
        ...
        try:
            ...
            stdout, stderr, exit_code = ssh_client.exec_command("sudo ipmitool lan print 1")
            
            ssh_client.close()
            
            # Update database with IPMI configuration
            if context.db_helper:
                context.db_helper.updateserverinfo(context.server_id, 'ipmi_address', context.target_ipmi_ip)
                context.db_helper.updateserverinfo(context.server_id, 'kcs_status', 'Configured')
            ...
            if context.db_helper:
                context.db_helper.updateserverinfo(context.server_id, 'kcs_status', f'Configuration Failed: {str(e)}')
    ...
    def _finalize_server_basic(self, context: WorkflowContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if context.db_helper:
                context.db_helper.updateserverinfo(context.server_id, 'status_name', 'Basic Commissioning Complete')
                context.db_helper.updateserverinfo(context.server_id, 'is_ready', 'TRUE')
            ...
            # Update database with error status
            if context.db_helper:
                context.db_helper.updateserverinfo(context.server_id, 'status_name', f'Finalization Error: {str(e)}')
    ...
    def _get_next_steps(self, context: WorkflowContext) -> List[str]:
        ...
        if not context.target_ipmi_ip:
            if hasattr(context, 'discovered_ipmi_ip') and context.discovered_ipmi_ip:
                next_steps.append(f"Configure IPMI using discovered IP: {context.discovered_ipmi_ip}")
            else:
    ...
    def _finalize_server(self, context: WorkflowContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if context.db_helper:
                context.db_helper.updateserverinfo(context.server_id, 'status_name', 'Provisioning Complete')
                context.db_helper.updateserverinfo(context.server_id, 'is_ready', 'TRUE')
                
                # Store additional metadata if available
                if context.hardware_discovery_result:
                    vendor_info = context.hardware_discovery_result.get('vendor_info', {})
                    if vendor_info:
                        # Store vendor-specific information in a serialized format
                        vendor_summary = f"Vendor: {vendor_info.get('manufacturer', 'Unknown')}"
                        if 'bmc_version' in vendor_info:
                            vendor_summary += f", BMC: {vendor_info['bmc_version']}"
                        context.db_helper.updateserverinfo(context.server_id, 'currServerModels', vendor_summary)
            ...
            logger.info(f"Server {context.server_id} provisioning completed successfully")
            return {
                'status': 'completed',
                'tags_applied': tags,
                'metadata': context.metadata,
    ...
    def configure_server_ipmi(
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(f"SELECT ip_address FROM {self.manager.db_helper.tablename} WHERE server_id = ?", (server_id,))
                    ...
                    if interface_set:  # Check if interface_set is not None
                        for interface in interface_set:
                            if interface and 'links' in interface:
                                ...
                                if links_list:  # Check if links is not None
                                    for link in links_list:
                                        if link and link.get('mode') == 'auto' and link.get('ip'):
            ...
            if not context.server_ip:
                raise Exception(f"Could not determine IP address for server {server_id}")
            ...
            self.manager.db_helper.updateserverinfo(server_id, 'status_name', 'Fully Configured')
            if rack_location:
                self.manager.db_helper.updateserverinfo(server_id, 'currServerModels', 
            ...
            if self.manager.db_helper:
                self.manager.db_helper.updateserverinfo(server_id, 'status_name', f'IPMI Config Error: {str(e)}')
        ...
        try:
            if context.db_helper and context.workflow_id:
                ...
                context.db_helper.execute_query("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (context.workflow_id, context.server_id, 'commissioning', context.device_type, 'running', 
