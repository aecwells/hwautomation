...
class PullBiosConfigStep(BaseWorkflowStep):
    ...
    def execute(self, context: StepContext) -> StepExecutionResult:
        ...
        try:
            ...
            context.add_sub_task(f"Pulling BIOS configuration from {context.manufacturer} server")
            ...
            return StepExecutionResult.success(
                "BIOS configuration pulled successfully",
                {"bios_config": config_result}
            )
            
        except Exception as e:
            return StepExecutionResult.failure(f"Failed to pull BIOS configuration: {e}")
    ...
    def _pull_supermicro_config(self, context: StepContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if not result["success"]:
                return {"success": False, "error": f"SUM command failed: {result.get('stderr', 'Unknown error')}"}
            ...
            if not download_result["success"]:
                return {"success": False, "error": "Failed to download BIOS configuration"}
...
class ModifyBiosConfigStep(BaseWorkflowStep):
    ...
    def execute(self, context: StepContext) -> StepExecutionResult:
        ...
        try:
            context.add_sub_task(f"Modifying BIOS configuration for device type: {context.device_type}")
            ...
            context.add_sub_task(f"BIOS configuration modified with {len(result.changes_made)} changes")
            ...
            return StepExecutionResult.failure(f"Failed to modify BIOS configuration: {e}")
...
class PushBiosConfigStep(RetryableWorkflowStep):
    ...
    def _execute_with_retry(self, context: StepContext) -> StepExecutionResult:
        ...
        try:
            ...
            return StepExecutionResult.success(
                "BIOS configuration pushed successfully",
                {"push_result": result}
            )
            
        except Exception as e:
            return StepExecutionResult.retry(f"Exception pushing BIOS configuration: {e}")
    ...
    def execute(self, context: StepContext) -> StepExecutionResult:
        ...
        try:
            ...
            if push_result.success and push_result.config_applied:
                context.add_sub_task(f"BIOS configuration verified - {len(changes_made)} changes applied")
            ...
            return StepExecutionResult.failure(f"BIOS configuration verification failed: {e}")
...
class RecordBiosConfigStep(BaseWorkflowStep):
    ...
    def execute(self, context: StepContext) -> StepExecutionResult:
        ...
        try:
            ...
            db_helper.updateserverinfo(context.server_id, "device_type", context.device_type)
            
            # Record configuration details
            changes_made = context.get_data("bios_changes", [])
            if changes_made:
                db_helper.updateserverinfo(context.server_id, "bios_changes_count", str(len(changes_made)))
                db_helper.updateserverinfo(context.server_id, "bios_changes", json.dumps(changes_made))
            
            # Record push result
            push_result = context.get_data("bios_push_result")
            if push_result:
                db_helper.updateserverinfo(context.server_id, "bios_config_status", "applied")
                if hasattr(push_result, 'applied_at'):
                    db_helper.updateserverinfo(context.server_id, "bios_config_applied_at", str(push_result.applied_at))
            ...
            return StepExecutionResult.failure(f"Failed to record BIOS configuration results: {e}")
