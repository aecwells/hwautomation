...
class EstablishSSHConnectionStep(RetryableWorkflowStep):
    ...
    def __init__(self, ssh_username: str = "ubuntu", ssh_key_path: Optional[str] = None):
        super().__init__(
    ...
    def _execute_with_retry(self, context: StepContext) -> StepExecutionResult:
        ...
        try:
            ...
            return StepExecutionResult.retry(f"Exception establishing SSH connection: {e}")
...
class TestNetworkConnectivityStep(BaseWorkflowStep):
    ...
    def execute(self, context: StepContext) -> StepExecutionResult:
        ...
        try:
            ...
            overall_status = "good" if connectivity_results.get("internet") else "limited"
            
            context.add_sub_task(f"Network connectivity test completed: {overall_status}")
            
            return StepExecutionResult.success(
                f"Network connectivity tested: {overall_status}",
                {"connectivity_status": overall_status, "details": connectivity_results}
            )
...
class ConfigureNetworkSettingsStep(ConditionalWorkflowStep):
    ...
    def _execute_conditional(self, context: StepContext) -> StepExecutionResult:
        ...
        try:
            ...
            return StepExecutionResult.success(
                f"Network configured with gateway {gateway}",
                {"gateway": gateway}
            )
    ...
    def _configure_gateway(self, ssh_client: SSHClient, gateway: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = ssh_client.execute_command(f"sudo ip route add default via {gateway}")
            ...
            if not verify_result.get("success") or gateway not in verify_result.get("stdout", ""):
                return {
                    "success": False,
                    "error": "Gateway route verification failed"
                }
...
class ValidateNetworkConfigurationStep(BaseWorkflowStep):
    ...
    def execute(self, context: StepContext) -> StepExecutionResult:
        ...
        try:
            ...
            validation_results["gateway_configured"] = gateway_result.get("success", False)
            ...
            validation_results["external_connectivity"] = connectivity_result.get("success", False)
    ...
    def execute(self, context: StepContext) -> StepExecutionResult:
        ...
        try:
            ...
            for interface in inventory.get("interfaces", []):
                ...
                mac_cmd = f"cat /sys/class/net/{interface}/address 2>/dev/null || echo 'N/A'"
            ...
            # Get default gateway
            gateway_result = ssh_client.execute_command("ip route show default | awk '{print $3}' | head -1")
            ...
            return StepExecutionResult.success(
                "Network inventory gathered",
                {"inventory": inventory}
            )
            
        except Exception as e:
            return StepExecutionResult.failure(f"Network inventory gathering failed: {e}")
...
class PingTestStep(BaseWorkflowStep):
    ...
    def __init__(self, targets: Optional[List[str]] = None):
        super().__init__(
            name="ping_test",
            description="Perform ping tests to verify connectivity"
        )
    ...
    def execute(self, context: StepContext) -> StepExecutionResult:
        ...
        try:
            context.add_sub_task(f"Performing ping tests to {len(self.targets)} targets")
            ...
            successful_pings = sum(1 for result in ping_results.values() if result["success"])
            success_rate = successful_pings / len(self.targets)
            
            context.add_sub_task(f"Ping tests completed: {successful_pings}/{len(self.targets)} successful")
        ...
        try:
            ...
            if inventory:
                if inventory.get("hostname"):
                    db_helper.updateserverinfo(context.server_id, "hostname", inventory["hostname"])
                
                if inventory.get("ip_addresses"):
                    primary_ip = inventory["ip_addresses"][0] if inventory["ip_addresses"] else None
                    if primary_ip:
                        db_helper.updateserverinfo(context.server_id, "primary_ip", primary_ip)
                
                if inventory.get("default_gateway"):
                    db_helper.updateserverinfo(context.server_id, "default_gateway", inventory["default_gateway"])
                ...
                for interface in interfaces:
                    ...
                    if inventory.get(mac_key):
                        db_helper.updateserverinfo(context.server_id, f"mac_{interface}", inventory[mac_key])
                        break  # Record primary interface MAC
            
            # Record connectivity test results
            connectivity = context.get_data("network_connectivity")
            if connectivity:
                db_helper.updateserverinfo(context.server_id, "internet_connectivity", 
                                         "yes" if connectivity.get("internet") else "no")
                db_helper.updateserverinfo(context.server_id, "dns_resolution", 
                                         "yes" if connectivity.get("dns") else "no")
            
            # Record ping test results
            ping_results = context.get_data("ping_test_results")
            if ping_results:
                successful_targets = [target for target, result in ping_results.items() if result["success"]]
                db_helper.updateserverinfo(context.server_id, "ping_test_targets", str(len(successful_targets)))
            ...
            return StepExecutionResult.failure(f"Failed to record network information: {e}")
