    ...
    def execute(self, context: StepContext) -> StepExecutionResult:
        ...
        try:
            ...
            if hardware_info:
                ...
                context.set_data("hardware_info", {
                ...
                context.add_sub_task(f"Discovered {hardware_info.manufacturer} {hardware_info.model}")
...
class DetectServerVendorStep(BaseWorkflowStep):
    ...
    def execute(self, context: StepContext) -> StepExecutionResult:
        ...
        try:
            ...
            if vendor:
                ...
                return StepExecutionResult.success(
                    f"Server vendor detected: {vendor}",
                    {"vendor": vendor}
                )
            else:
    ...
    def _detect_vendor(self, ssh_client: SSHClient) -> Optional[str]:
        ...
        vendor_commands = [
            ...
            ("ipmitool mc info | grep 'Manufacturer Name'", r"Supermicro|Dell|HP|HPE|Lenovo"),
        ]
...
class GatherSystemInfoStep(BaseWorkflowStep):
    ...
    def _gather_system_info(self, ssh_client: SSHClient) -> Dict[str, Any]:
        ...
        for key, command in info_commands.items():
            try:
                ...
                if result["success"] and result["stdout"]:
                    ...
                    if value and value.lower() not in ["not specified", "to be filled by o.e.m.", "unknown"]:
        ...
        # Network interfaces
        try:
            result = ssh_client.execute_command("ip -o link show | grep -E 'eth|ens|enp'")
            if result["success"] and result["stdout"]:
                interfaces = []
    ...
    def execute(self, context: StepContext) -> StepExecutionResult:
        ...
        try:
            ...
            if context.manufacturer:
                db_helper.updateserverinfo(context.server_id, "manufacturer", context.manufacturer)
            
            if context.model:
                db_helper.updateserverinfo(context.server_id, "model", context.model)
            
            if context.serial_number:
                db_helper.updateserverinfo(context.server_id, "serial_number", context.serial_number)
            ...
            if hardware_info:
                db_helper.updateserverinfo(context.server_id, "hardware_discovery", "completed")
                ...
                if network_interfaces:
                    ...
                    db_helper.updateserverinfo(context.server_id, "network_interfaces", ",".join(interface_names))
            ...
            return StepExecutionResult.failure(f"Failed to record hardware information: {e}")
