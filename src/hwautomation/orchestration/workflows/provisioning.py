...
from ..steps.network_config import EstablishSSHConnectionStep as NetworkSSHStep
from ..steps.network_config import (
...
class ServerProvisioningWorkflow(BaseWorkflow):
    ...
    def build_steps(self) -> List[BaseWorkflowStep]:
        ...
        steps.append(
            NetworkSSHStep(
                ssh_username=self.ssh_username, ssh_key_path=self.ssh_key_path
            )
        )
    ...
    def get_workflow_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ipmi_range": (
                f"{self.ipmi_range_start} - {self.ipmi_range_end}"
                if self.ipmi_range_start
                else None
            ),
