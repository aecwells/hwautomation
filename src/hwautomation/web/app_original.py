...
def create_app():
    ...
    db_helper = DbHelper(
        tablename=table_name, db_path=db_path, auto_migrate=auto_migrate
    )
        ...
        try:
            ...
            for i, device_id in enumerate(device_ids):
                try:
                    ...
                    results.append(
                        {"device_id": device_id, "status": "failed", "error": str(e)}
                    )
    ...
    def api_orchestration_workflow_cancel(workflow_id):
        ...
        try:
            ...
            if not success:
                return (
                    jsonify({"error": "Workflow not found or cannot be cancelled"}),
                    404,
                )
        ...
        try:
            ...
            def execute_workflow():
                try:
                    ...
                    logger.info(
                        f"Workflow {workflow.id} completed with success: {success}"
                    )
                except Exception as e:
    ...
    def api_start_firmware_first_provisioning():
        ...
        try:
            ...
            workflow = (
                provisioning_workflow.create_firmware_first_provisioning_workflow(
                    server_id=server_id,
                    device_type=device_type,
                    target_ipmi_ip=target_ipmi_ip,
                    rack_location=rack_location,
            ...
            def execute_firmware_workflow():
                try:
                    logger.info(
                        f"Starting firmware-first provisioning workflow {workflow.id} for server {server_id}"
                    )
                    result = workflow.execute(context)
                    logger.info(
                        f"Firmware-first provisioning workflow {workflow.id} completed with result: {result}"
                    )

                    # Emit completion event
                    socketio.emit(
                        "firmware_workflow_complete",
                        {
                    ...
                    logger.error(
                        f"Firmware-first provisioning workflow {workflow.id} failed: {e}"
                    )
                    import traceback
