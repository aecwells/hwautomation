...
orchestration_ns = Namespace('orchestration', description='Workflow orchestration and server provisioning')
...
health_model = api.model('Health', {
    'status': fields.String(required=True, description='System health status', example='healthy'),
    'service': fields.String(required=True, description='Service identifier', example='hwautomation-web'),
...
error_model = api.model('Error', {
    'success': fields.Boolean(required=True, description='Operation success flag', example=False),
    'error': fields.String(required=True, description='Error message', example='Operation failed'),
    'error_code': fields.String(description='Machine-readable error code', example='VALIDATION_ERROR'),
    'details': fields.Raw(description='Additional error details')
})

success_model = api.model('Success', {
    'success': fields.Boolean(required=True, description='Operation success flag', example=True),
    'message': fields.String(description='Success message', example='Operation completed'),
...
workflow_status_model = api.model('WorkflowStatus', {
    ...
    'status': fields.String(required=True, description='Current workflow status', 
                          enum=['PENDING', 'RUNNING', 'COMPLETED', 'FAILED', 'CANCELLED']),
    'progress': fields.Integer(description='Completion percentage (0-100)', min=0, max=100),
...
workflow_list_model = api.model('WorkflowList', {
    'workflows': fields.List(fields.Nested(workflow_status_model), description='List of workflows'),
...
provision_request_model = api.model('ProvisionRequest', {
    'server_id': fields.String(required=True, description='Server identifier', example='node-123'),
    'device_type': fields.String(required=True, description='Device type configuration', 
                                example='a1.c5.large'),
    'target_ipmi_ip': fields.String(description='Target IPMI IP address', example='192.168.1.100'),
    'gateway': fields.String(description='Network gateway IP', example='192.168.1.1'),
    'rack_location': fields.String(description='Physical rack location', example='Rack-A-U12')
})

provision_response_model = api.model('ProvisionResponse', {
    'success': fields.Boolean(required=True, description='Operation success flag'),
    'workflow_id': fields.String(required=True, description='Created workflow identifier'),
...
# Database models
database_info_model = api.model('DatabaseInfo', {
    'success': fields.Boolean(required=True, description='Operation success flag'),
    'info': fields.Raw(required=True, description='Database information', example={
...
# MaaS models
maas_discovery_model = api.model('MaaSDiscovery', {
...
log_entry_model = api.model('LogEntry', {
    ...
    'component': fields.String(required=True, description='Component that generated the log'),
...
class ProvisionServer(Resource):
    ...
    @orchestration_ns.doc('provision_server',
...
class WorkflowList(Resource):
    ...
    @orchestration_ns.doc('list_workflows',
                          description='Get list of all workflows with their status',
                          responses={
                              200: 'Success - Workflows retrieved',
                              500: 'Internal Server Error'
                          })
    def get(self):
...
class WorkflowStatus(Resource):
    ...
    @orchestration_ns.doc('get_workflow_status',
...
class WorkflowCancel(Resource):
    ...
    @orchestration_ns.doc('cancel_workflow',
...
class DatabaseInfo(Resource):
    ...
    @database_ns.doc('get_database_info',
...
class MaaSDiscover(Resource):
    ...
    @maas_ns.doc('discover_maas_devices',
...
def handle_value_error(error):
    ...
    return {'success': False, 'error': str(error), 'error_code': 'VALIDATION_ERROR'}, 400

@api.errorhandler(KeyError)
def handle_key_error(error):
    """Handle missing parameter errors"""
    return {'success': False, 'error': f'Missing required parameter: {str(error)}', 'error_code': 'MISSING_PARAMETER'}, 400

@api.errorhandler(Exception)
def handle_generic_error(error):
    """Handle unexpected errors"""
    logger.exception("Unexpected API error")
    return {'success': False, 'error': 'Internal server error', 'error_code': 'INTERNAL_ERROR'}, 500
