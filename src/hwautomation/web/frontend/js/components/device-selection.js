...
class DeviceSelectionComponent {
    ...
    async initialize() {
        ...
        this.currentView = this.stateManager.getState('devices.currentView', 'cards');
        this.applyView();
    ...
    initializeModals() {
        if (this.elements.deviceDetailsModal && this.elements.commissionModal) {
            if (typeof bootstrap !== 'undefined') {
                this.deviceDetailsModal = new bootstrap.Modal(this.elements.deviceDetailsModal);
                this.commissionModal = new bootstrap.Modal(this.elements.commissionModal);
            } else {
    ...
    async loadDevices() {
        try {
            ...
            if (response && response.machines) {
                ...
                this.renderDevices();
                this.notificationService.success(`Loaded ${this.currentDevices.length} devices`);
            } else {
                this.currentDevices = [];
                this.renderDevices();
            ...
            this.notificationService.error('Failed to load devices: ' + error.message);
            this.currentDevices = [];
            this.renderDevices();
        } finally {
    ...
    renderCardsView() {
        ...
        if (this.currentDevices.length === 0) {
            container.innerHTML = '<div class="col-12"><div class="alert alert-info">No devices found</div></div>';
            return;
        }
    ...
    async showDeviceDetails(deviceId) {
        ...
        try {
            ...
            const details = await this.apiService.get(`/api/maas/machines/${deviceId}`);
    ...
    applyView() {
        ...
        const cardsContainer = this.elements['device-cards-container']?.closest('.devices-cards-view');
        const listContainer = this.elements['device-list-container']?.closest('.devices-list-view');

        if (cardsContainer) {
            cardsContainer.style.display = this.currentView === 'cards' ? 'block' : 'none';
        }

        if (listContainer) {
            listContainer.style.display = this.currentView === 'list' ? 'block' : 'none';
        }
    }
