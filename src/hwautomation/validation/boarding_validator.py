...
class BoardingValidation:
    ...
    def update_summary(self):
        ...
        self.summary = {
            ...
            'passed': len([v for v in self.validations if v.status == ValidationStatus.PASS]),
            'failed': len([v for v in self.validations if v.status == ValidationStatus.FAIL]),
            'warnings': len([v for v in self.validations if v.status == ValidationStatus.WARNING]),
            'skipped': len([v for v in self.validations if v.status == ValidationStatus.SKIP])
        }
...
class BMCBoardingValidator:
    ...
    def _load_bios_requirements(self) -> Dict[str, Dict]:
        ...
        return {
            'rocketlake': {
                ...
                'boot': {
                    'boot_mode': 'uefi',
                    'boot_order': ['UEFI USB CD/DVD', 'UEFI USB Key', 'UEFI USB Hard Disk', 'UEFI Hard Disk', 'UEFI Network']
                },
                'advanced': {
            ...
            'icelake_spr': {
                ...
                'boot': {
                    'boot_mode': 'uefi',
                    'boot_order': ['UEFI USB Key', 'UEFI USB CD/DVD', 'UEFI USB Hard Disk', 'UEFI Hard Disk', 'UEFI Network']
                },
                'advanced': {
    ...
    def validate_complete_boarding(self, device_id: str, device_type: str, 
                                 server_ip: str, ipmi_ip: str) -> BoardingValidation:
        """
        ...
        logger.info(f"Starting complete boarding validation for {device_id} ({device_type})")
        
        # 1. Validate basic connectivity
        validation.validations.extend(self._validate_connectivity(device_id, server_ip, ipmi_ip))
        
        # 2. Validate hardware information
        validation.validations.extend(self._validate_hardware_info(device_id, server_ip))
        
        # 3. Validate IPMI configuration
        validation.validations.extend(self._validate_ipmi_configuration(device_id, ipmi_ip, device_type))
        
        # 4. Validate BIOS configuration (if accessible)
        validation.validations.extend(self._validate_bios_configuration(device_id, server_ip, device_type))
        
        # 5. Validate network configuration
        validation.validations.extend(self._validate_network_configuration(device_id, server_ip))
        
        # 6. Validate device page requirements
        validation.validations.extend(self._validate_device_page_requirements(device_id, device_type))
        
        # Update summary and overall status
        validation.update_summary()
        
        logger.info(f"Boarding validation complete for {device_id}: {validation.overall_status.value} "
                   f"({validation.summary['passed']}/{validation.summary['total']} passed)")
        
        return validation
    
    def _validate_connectivity(self, device_id: str, server_ip: str, ipmi_ip: str) -> List[ValidationResult]:
    ...
    def _validate_hardware_info(self, device_id: str, server_ip: str) -> List[ValidationResult]:
        ...
        try:
            ...
            disk_info = ssh_client.execute_command("lsblk -d -o NAME,SIZE,MODEL | grep -v loop")
            if disk_info:
                results.append(ValidationResult(
            ...
            nic_info = ssh_client.execute_command("ip link show | grep -E '^[0-9]+:' | grep -v lo")
            if nic_info:
    ...
    def _validate_ipmi_configuration(self, device_id: str, ipmi_ip: str, device_type: str) -> List[ValidationResult]:
        ...
        try:
            ...
            cmd = ['ipmitool', '-I', 'lanplus', '-H', ipmi_ip, '-U', 'ADMIN', '-P', admin_password, 'mc', 'info']
            ...
            # Test power control
            cmd = ['ipmitool', '-I', 'lanplus', '-H', ipmi_ip, '-U', 'ADMIN', '-P', admin_password, 'power', 'status']
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=10)
            ...
            # Test sensor reading
            cmd = ['ipmitool', '-I', 'lanplus', '-H', ipmi_ip, '-U', 'ADMIN', '-P', admin_password, 'sensor', 'list']
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=15)
            
            if result.returncode == 0:
                sensor_count = len([line for line in result.stdout.split('\n') if line.strip() and '|' in line])
                results.append(ValidationResult(
    ...
    def _validate_bios_configuration(self, device_id: str, server_ip: str, device_type: str) -> List[ValidationResult]:
        ...
        try:
            ...
            boot_mode = ssh_client.execute_command("[ -d /sys/firmware/efi ] && echo 'UEFI' || echo 'Legacy'")
            ...
            secure_boot = ssh_client.execute_command("mokutil --sb-state 2>/dev/null || echo 'Not available'")
            ...
            if 'sgx' in device_type.lower() or any(x in device_type for x in ['s5', 'icx', 'spr']):
                sgx_status = ssh_client.execute_command("grep -i sgx /proc/cpuinfo | head -1")
                if sgx_status:
                    results.append(ValidationResult(
    ...
    def _validate_network_configuration(self, device_id: str, server_ip: str) -> List[ValidationResult]:
        ...
        try:
            ...
            interfaces = ssh_client.execute_command("ip link show | grep -E '^[0-9]+:' | grep -v lo | wc -l")
            if interfaces:
            ...
            active_interfaces = ssh_client.execute_command("ip link show up | grep -E '^[0-9]+:' | grep -v lo | wc -l")
            ...
            dns_test = ssh_client.execute_command("nslookup google.com >/dev/null 2>&1 && echo 'OK' || echo 'FAIL'")
    ...
    def _validate_device_page_requirements(self, device_id: str, device_type: str) -> List[ValidationResult]:
        ...
        if any(prefix in device_type for prefix in ['d1.', 'd2.', 'd3.', 's1.', 's2.', 's3.', 's4.', 's5.', 'hp.']):
            results.append(ValidationResult(
