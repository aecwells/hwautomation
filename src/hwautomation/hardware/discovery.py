...
from ..utils.network import SSHManager, SSHClient
...
class HardwareDiscoveryManager:
    ...
    def discover_hardware(self, host: str, username: str = "ubuntu", 
                         key_file: str = None) -> HardwareDiscovery:
        """
    ...
    def _discover_system_info(self, ssh_client: SSHClient, errors: List[str]) -> SystemInfo:
        """Discover system hardware information using dmidecode and vendor tools"""
        system_info = SystemInfo()
        
        try:
            # Get system information from dmidecode
            stdout, stderr, exit_code = ssh_client.exec_command("sudo dmidecode -t system")
            if exit_code == 0:
            ...
            stdout, stderr, exit_code = ssh_client.exec_command("sudo dmidecode -t bios")
            if exit_code == 0:
            ...
            if system_info.manufacturer:
                vendor_info = self._discover_vendor_specific_info(ssh_client, system_info.manufacturer, errors)
    ...
    def _discover_ipmi_info(self, ssh_client: SSHClient, errors: List[str]) -> IPMIInfo:
        ...
        try:
            ...
            if exit_code != 0:
                ...
                install_stdout, stderr, exit_code = ssh_client.exec_command("sudo apt-get update && sudo apt-get install -y ipmitool")
            ...
            if exit_code == 0:
                ...
                stdout, stderr, exit_code = ssh_client.exec_command("sudo ipmitool lan print 8")
                if exit_code == 0:
                    ipmi_info = self._parse_ipmi_lan_config(stdout)
                    ipmi_info.enabled = True
                    ipmi_info.channel = 8
    ...
    def _discover_vendor_specific_info(self, ssh_client: SSHClient, manufacturer: str, errors: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'supermicro' in manufacturer_lower or 'super micro' in manufacturer_lower:
                vendor_info.update(self._discover_supermicro_info(ssh_client, errors))
            elif 'hewlett' in manufacturer_lower or 'hpe' in manufacturer_lower or 'hp ' in manufacturer_lower:
    ...
    def _discover_supermicro_info(self, ssh_client: SSHClient, errors: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            stdout, stderr, exit_code = ssh_client.exec_command("sudo sumtool -c GetSystemInfo")
            if exit_code == 0:
            ...
            stdout, stderr, exit_code = ssh_client.exec_command("sudo sumtool -c GetBiosInfo")
            if exit_code == 0:
            ...
            stdout, stderr, exit_code = ssh_client.exec_command("sudo sumtool -c GetBmcInfo")
            if exit_code == 0:
    ...
    def _discover_hpe_info(self, ssh_client: SSHClient, errors: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if hpe_tool:
                stdout, stderr, exit_code = ssh_client.exec_command(f"sudo {hpe_tool} ctrl all show config")
                if exit_code == 0:
            ...
            stdout, stderr, exit_code = ssh_client.exec_command("sudo dmidecode -t 38 | grep -i ilo")
    ...
    def _discover_dell_info(self, ssh_client: SSHClient, errors: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            stdout, stderr, exit_code = ssh_client.exec_command("sudo omreport chassis info")
            if exit_code == 0:
    ...
    def _install_sumtool(self, ssh_client: SSHClient, errors: List[str]) -> bool:
        ...
        try:
            ...
            stdout, stderr, exit_code = ssh_client.exec_command("sudo apt-get install -y wget alien", timeout=300)
            if exit_code != 0:
    ...
    def _install_dell_tools(self, ssh_client: SSHClient, errors: List[str]) -> bool:
        ...
        try:
            ...
            # Start services
            ssh_client.exec_command("sudo /opt/dell/srvadmin/sbin/srvadmin-services.sh start", timeout=60)
    ...
    def _discover_network_interfaces(self, ssh_client: SSHClient, errors: List[str]) -> List[NetworkInterface]:
    ...
    def find_ipmi_address(self, host: str, username: str = "ubuntu", 
            ...
            for ip in network.hosts():
                ...
                try:
                    ...
                    if ipmi_addr:
                        ...
                        self.logger.info(f"Found IPMI address {ipmi_addr} for host {ip_str}")
                except Exception as e:
