...
class OperationProgress:
    ...
    def estimated_completion(self) -> Optional[datetime]:
        ...
        if self.progress_percentage > 0 and self.status == OperationStatus.RUNNING:
            ...
            if duration:
                total_estimated = duration.total_seconds() / (self.progress_percentage / 100.0)
...
class ProgressCallback:
    ...
    async def on_operation_status_change(self, operation_id: str, status: OperationStatus):
...
class BiosConfigMonitor:
    ...
    async def cancel_operation(self, operation_id: str, reason: str = "Operation cancelled"):
        """Cancel an operation"""
        if operation_id not in self.operations:
            raise ValueError(f"Operation {operation_id} not found")
    ...
    async def start_subtask(self, operation_id: str, subtask_name: str, 
                          description: str = ""):
    ...
    async def update_progress(self, operation_id: str, percentage: float, 
    ...
    async def log_info(self, operation_id: str, info_message: str, 
...
async def create_monitored_operation(operation_type: str = "bios_configuration",
                                   metadata: Optional[Dict[str, Any]] = None) -> str:
