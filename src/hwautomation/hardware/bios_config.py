...
class BiosConfigManager:
        ...
        if preserve_file.exists():
            try:
                ...
                self.preserve_settings = set(
                    preserve_config.get("preserve_settings", [])
                )
    ...
    def apply_template_to_config(
        self, current_config: ET.Element, device_type: str
        ...
        template_rules = self.template_rules["template_rules"][device_type][
            "modifications"
        ]
        changes_made = []
        ...
        current_settings = {
            setting.get("name"): setting
            for setting in current_config.findall("Setting")
        }
        ...
        for rule_name, rule_value in template_rules.items():
            ...
            if existing_setting is not None:
                ...
                if old_value != rule_value:
                    ...
                    changes_made.append(
                        f"Changed {rule_name}: {old_value} -> {rule_value}"
                    )
    ...
    def validate_modified_config(
        self, original_config: ET.Element, modified_config: ET.Element, device_type: str
    ) -> Tuple[bool, List[str]]:
        """
            ...
            if self._should_preserve_setting(setting_name):
                if setting_name in modified_settings:
                    if (
                        ...
                        != modified_settings[setting_name]
                    ):
                    ...
                    validation_errors.append(
                        f"Preserved setting {setting_name} was removed"
                    )
        ...
        if device_config and "required_settings" in device_config:
            for required_setting in device_config["required_settings"]:
                if required_setting not in modified_settings:
                    validation_errors.append(
                        f"Required setting {required_setting} is missing"
                    )
    ...
    def _check_setting_conflicts(self, settings: Dict[str, str]) -> List[str]:
        ...
        if (
            settings.get("BootMode") == "Legacy"
            and settings.get("SecureBoot") == "Enabled"
        ):
            conflicts.append("Secure Boot cannot be enabled in Legacy boot mode")

        if (
            settings.get("IntelVT") == "Disabled"
            and settings.get("IntelVTD") == "Enabled"
        ):
    ...
    def push_modified_config(
        self,
        modified_config: ET.Element,
        target_ip: str,
        username: str = "ADMIN",
        password: str = None,
        backup: bool = True,
    ...
    def apply_bios_config_smart(
        ...
        try:
            ...
            current_config = self.pull_current_bios_config(
            ...
            modified_config, changes_made = self.apply_template_to_config(
                current_config, device_type
            )
            ...
            is_valid, validation_errors = self.validate_modified_config(
                current_config, modified_config, device_type
            )
            ...
            push_success = self.push_modified_config(
            ...
            if push_success:
                ...
                logger.info(
                    f"Successfully applied BIOS configuration for {device_type} to {target_ip}"
                )
            else:
    ...
    def set_bios_settings_via_redfish(
        self,
        target_ip: str,
    ...
    def determine_bios_config_method(
        ...
        try:
            ...
            if not redfish_available:
                logger.info(
                    f"Redfish not available for {target_ip}, using vendor tools"
                )
                return "vendor_tool"
            ...
            device_config = self.get_device_config(device_type)
            if device_config:
                preferred_method = device_config.get(
                ...
                if preferred_method == "redfish":
                    ...
                    with RedfishManager(target_ip, username, password) as redfish:
                        ...
                        if capabilities.supports_bios_config:
                            logger.info(
                                f"Using Redfish for {target_ip} (preferred and capable)"
                            )
                            return "redfish"
                        else:
                            logger.info(
                                f"Redfish preferred but BIOS config not supported, using vendor tools"
                            )
                            return "vendor_tool"
    ...
    def apply_bios_config_smart_enhanced(
        self,
        device_type: str,
        target_ip: str,
        username: str = "ADMIN",
        ...
        try:
            ...
            if prefer_redfish:
                method = self.determine_bios_config_method(
            ...
            if method == "redfish":
                ...
                return self._apply_bios_config_via_redfish(
                    device_type, target_ip, username, password, dry_run, result
                )

            elif method == "hybrid":
                logger.info(
                    f"Applying BIOS configuration via hybrid approach for {target_ip}"
                )
                return self._apply_bios_config_hybrid(
                    device_type, target_ip, username, password, dry_run, result
                )

            else:  # vendor_tool
                logger.info(
                    f"Applying BIOS configuration via vendor tools for {target_ip}"
                )
                # Fall back to existing smart method
                vendor_result = self.apply_bios_config_smart(
                    device_type, target_ip, username, password, dry_run
                )
    ...
    def apply_bios_config_phase2(
        ...
        try:
            ...
            if not device_config:
                result["error"] = (
                    f"Device type {device_type} not found in configuration"
                )
                return result
            ...
            template_rules = self.template_rules.get("template_rules", {}).get(
                device_type, {}
            )
            ...
            if not settings_to_apply:
                result["error"] = (
                    f"No BIOS settings found for device type: {device_type}"
                )
                return result

            # Analyze settings and determine optimal methods
            logger.info(
                f"Analyzing {len(settings_to_apply)} BIOS settings for optimal method selection"
            )
            ...
            logger.info(
                f"Method analysis complete: {len(method_analysis.redfish_settings)} Redfish, "
            ...
            if dry_run:
                ...
                result["dry_run_summary"] = {
                    ...
                    "estimated_total_time": method_analysis.performance_estimate.get(
            ...
            if method_analysis.redfish_settings:
                logger.info(
                    f"Applying {len(method_analysis.redfish_settings)} settings via Redfish"
                )
            ...
            if method_analysis.vendor_settings:
                logger.info(
                    f"Applying {len(method_analysis.vendor_settings)} settings via vendor tools"
                )
                vendor_result = self._apply_settings_via_vendor_tools(
                    ...
                    username,
                    password,
                    method_analysis.vendor_settings,
                )
            ...
            if method_analysis.unknown_settings:
                logger.warning(
                    f"Found {len(method_analysis.unknown_settings)} unknown settings - skipping"
                )
            ...
            if success:
                logger.info(
                    f"BIOS configuration (per-setting decision) completed successfully for {target_ip}"
                )
            else:
                logger.warning(
                    f"BIOS configuration (per-setting decision) completed with some issues for {target_ip}"
                )
        ...
        try:
            ...
            if monitor:
                ...
                await monitor.log_info(
                    operation_id,
                    f"Starting monitored BIOS configuration for {target_ip}",
                )
            ...
            if not validation_result.get("success", False):
                if monitor and operation_id:
                    ...
                    await monitor.complete_operation(
                        operation_id, False, "Pre-flight validation failed"
                    )
                result["error"] = (
                    f"Pre-flight validation failed: {validation_result.get('error')}"
                )
            ...
            if not method_analysis_result.get("success", False):
                if monitor and operation_id:
                    ...
                    await monitor.complete_operation(
                        operation_id, False, "Method analysis failed"
                    )
                result["error"] = (
                    f"Method analysis failed: {method_analysis_result.get('error')}"
                )
                return result
            ...
            if dry_run:
                if monitor and operation_id:
                    ...
                    await monitor.complete_operation(
                        operation_id, True, "Dry run completed successfully"
                    )
            ...
            result["error_recovery_actions"] = execution_result.get(
                "recovery_actions", []
            )
            ...
            overall_success = execution_result.get(
                "success", False
            ...
            if monitor and operation_id:
                final_message = (
                    ...
                    if overall_success
                    else "Monitored BIOS configuration completed with issues"
                )
                await monitor.complete_operation(
                    operation_id, overall_success, final_message
                )
            ...
            if overall_success:
                logger.info(
                    f"Monitored BIOS configuration completed successfully for {target_ip}"
                )
            else:
                logger.warning(
                    f"Monitored BIOS configuration completed with issues for {target_ip}"
                )
            ...
            if monitor and operation_id:
                await monitor.log_error(
                    operation_id, error_message, {"exception": str(e)}
                )
                await monitor.complete_operation(operation_id, False, error_message)

            return result

    async def _phase3_pre_flight_validation(
        self,
        device_type: str,
        target_ip: str,
        username: str,
        ...
        try:
            ...
            if operation_id:
                await monitor.log_info(
                    operation_id, "Testing basic network connectivity"
                )
            ...
            if not connectivity_result.get("success", False):
                result["error"] = (
                    f"Connectivity test failed: {connectivity_result.get('error')}"
                )
                return result
            ...
            if operation_id:
                await monitor.log_info(
                    operation_id, "Analyzing configuration requirements"
                )

            config_analysis = await self._analyze_configuration_requirements(
                device_type
            )
            ...
            if operation_id:
                await monitor.log_error(
                    operation_id, f"Pre-flight validation error: {e}"
                )
            return result

    async def _phase3_method_analysis(
        self,
        device_type: str,
    ...
    async def _phase3_execute_configuration(
        self,
        target_ip: str,
        username: str,
        password: str,
    ...
    async def _phase3_post_validation(
        self,
        target_ip: str,
        username: str,
        password: str,
        ...
        try:
            ...
            if redfish_settings:
                if operation_id:
                    await monitor.log_info(
                        operation_id,
                        f"Validating {len(redfish_settings)} Redfish settings",
                    )
            ...
            if vendor_settings:
                if operation_id:
                    await monitor.log_info(
                        operation_id,
                        f"Vendor settings validation noted ({len(vendor_settings)} settings)",
                    )
        ...
        try:
            ...
            if method == "redfish":
                ...
                if not result["success"]:
                    result["error"] = redfish_result.get(
                        "error", "Redfish execution failed"
                    )
                ...
                if not result["success"]:
                    result["error"] = vendor_result.get(
                        "error", "Vendor tool execution failed"
                    )
    ...
    async def _validate_redfish_settings(
        self,
        target_ip: str,
        username: str,
        password: str,
        ...
        try:
            with RedfishManager(target_ip, username, password) as redfish:
                ...
                if current_settings:
                    ...
                    result["error"] = (
                        "Failed to retrieve current BIOS settings for validation"
                    )
    ...
    def _apply_settings_via_redfish(
        self, target_ip: str, username: str, password: str, settings: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            with RedfishManager(target_ip, username, password) as redfish:
                ...
                if update_result:
                    ...
                    logger.info(
                        f"Successfully applied {len(settings)} BIOS settings via Redfish"
                    )
                else:
    ...
    def _apply_settings_via_vendor_tools(
        self,
        device_type: str,
        target_ip: str,
        username: str,
        ...
        try:
            ...
            vendor_result = self.apply_bios_config_smart(
                device_type, target_ip, username, password, dry_run=False
            )
            ...
            if vendor_result.get("success", False):
                ...
                logger.info(
                    f"Successfully applied {len(settings)} BIOS settings via vendor tools"
                )
            else:
                result["error"] = vendor_result.get(
                    "error", "Unknown vendor tool error"
                )
                result["settings_failed"] = settings
                result["individual_results"] = vendor_result
    ...
    def validate_phase2_redfish_capabilities(
        self,
        device_type: str,
        target_ip: str,
        username: str = "ADMIN",
        ...
        try:
            ...
            with RedfishManager(target_ip, username, password) as redfish:
                ...
                if current_settings is None:
                    result["error"] = (
                        "Failed to retrieve BIOS settings from target system"
                    )
                    return result
                ...
                result["available_settings"] = [
                    setting
                    for setting, available in validation_results.items()
                    if available
                ]
                result["unavailable_settings"] = [
                    setting
                    for setting, available in validation_results.items()
                    if not available
                ]
                ...
                logger.info(
                    f"Redfish validation complete: {len(result['available_settings'])} available, "
    ...
    def _apply_bios_config_via_redfish(
        self,
        device_type: str,
        target_ip: str,
        username: str,
        ...
        try:
            with RedfishManager(target_ip, username, password) as redfish:
                ...
                if current_settings is None:
                    result["error"] = (
                        "Failed to retrieve current BIOS settings via Redfish"
                    )
                    return result
                ...
                if not template_modifications:
                    result["error"] = (
                        f"No template modifications found for device type: {device_type}"
                    )
                    return result
                ...
                for setting_name, target_value in template_modifications.items():
                    ...
                    if current_value != target_value:
                        ...
                        changes_made.append(
                            f"{setting_name}: {current_value} -> {target_value}"
                        )
                ...
                if dry_run:
                    result["success"] = True
                    result["message"] = (
                        f"Dry run completed - would apply {len(changes_to_apply)} changes via Redfish"
                    )
                    return result

                # Apply changes via Redfish
                logger.info(
                    f"Applying {len(changes_to_apply)} BIOS settings via Redfish"
                )
                ...
                if success:
                    result["success"] = True
                    result["message"] = (
                        f"Successfully applied {len(changes_to_apply)} BIOS settings via Redfish"
                    )
                    logger.info(
                        f"BIOS configuration applied successfully via Redfish for {target_ip}"
                    )
                else:
    ...
    def _apply_bios_config_hybrid(
        ...
        try:
            ...
            redfish_changes = {
                k: v for k, v in template_modifications.items() if k in redfish_settings
            }
            vendor_changes = {
                k: v
                for k, v in template_modifications.items()
                if k in vendor_only_settings
            }
            ...
            if redfish_changes:
                ...
                redfish_result = self._apply_bios_config_via_redfish(
                    device_type,
                    target_ip,
                    username,
                    password,
                ...
                if redfish_result.get("success"):
                    ...
                    logger.warning(
                        f"Redfish portion failed: {redfish_result.get('error', 'Unknown error')}"
                    )
            ...
            if vendor_changes:
                ...
                if vendor_result.get("success"):
                    ...
                    result["error"] = (
                        f"Vendor tool portion failed: {vendor_result.get('error', 'Unknown error')}"
                    )
                    return result
            ...
            result["success"] = True
            result["message"] = (
                f"Hybrid configuration completed - {len(all_changes)} total changes"
            )
            return result
    ...
    def list_templates(self) -> List[str]:
        ...
        logger.warning(
            "list_templates() is deprecated. Use get_device_types() instead."
        )
