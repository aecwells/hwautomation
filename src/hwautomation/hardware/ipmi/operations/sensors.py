...
class SensorManager:
    ...
    def get_sensor_data(self, credentials: IPMICredentials) -> List[SensorReading]:
        ...
        try:
            ...
            raise IPMICommandError(
                f"Sensor data collection timed out after {self.timeout}s",
                "sensor list"
            )
    ...
    def get_temperature_sensors(self, credentials: IPMICredentials) -> List[SensorReading]:
        ...
        try:
            ...
            for sensor in sensors:
                ...
                if (
                    "temp" in sensor.name.lower() or
                    (sensor.unit and "degrees" in sensor.unit.lower()) or
                    (sensor.unit and "c" in sensor.unit.lower())
                ):
    ...
    def get_power_sensors(self, credentials: IPMICredentials) -> List[SensorReading]:
        ...
        try:
            ...
            for sensor in sensors:
                ...
                if (
                    any(keyword in sensor.name.lower() for keyword in ["power", "watt", "volt", "amp"]) or
                    (sensor.unit and any(unit in sensor.unit.lower() for unit in ["w", "v", "a"]))
                ):
    ...
    def check_sensor_thresholds(self, credentials: IPMICredentials) -> Dict[str, List[str]]:
        ...
        try:
            ...
            for sensor in sensors:
                if sensor.status:
                    ...
                    elif any(keyword in status_lower for keyword in ["crit", "fail", "error"]):
