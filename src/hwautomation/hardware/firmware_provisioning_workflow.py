...
from .firmware_manager import (
    FirmwareManager, FirmwareType, FirmwareInfo, 
    FirmwareUpdateResult, UpdatePriority
)
...
class FirmwareProvisioningWorkflow:
    ...
    async def execute_firmware_first_provisioning(self, 
                                                 context: ProvisioningContext) -> ProvisioningResult:
        """
        ...
        if not context.operation_id:
            context.operation_id = self.monitor.create_operation("firmware_first_provisioning")
        ...
        try:
            ...
            await self.monitor.complete_subtask(context.operation_id, "pre_flight", True)
            ...
            if not context.skip_firmware_check:
                ...
                updates_needed = [fw for fw in context.firmware_info.values() if fw.update_required] \
                    if context.firmware_info else []
                critical_updates = [fw for fw in updates_needed if fw.priority == UpdatePriority.CRITICAL]
                
                await self.monitor.update_progress(context.operation_id, 20, 
                ...
                if updates_needed:
                    ...
                    successful_updates = [r for r in context.firmware_results if r.success] \
                        if context.firmware_results else []
                    failed_updates = [r for r in context.firmware_results if not r.success] \
                        if context.firmware_results else []
                    
                    await self.monitor.update_progress(context.operation_id, 50, 
                    ...
                    if reboot_required:
                        ...
                        await self.monitor.complete_subtask(context.operation_id, "system_reboot", True)
                else:
                    logger.info("No firmware updates required, proceeding to BIOS configuration")
                    await self.monitor.update_progress(context.operation_id, 70, 
            ...
            if not context.skip_bios_config:
                ...
                settings_applied = context.bios_results.get('settings_applied', 0) \
                    if context.bios_results else 0
                ...
                await self.monitor.complete_subtask(context.operation_id, "bios_config", True)
            else:
            ...
            await self.monitor.update_progress(context.operation_id, 100, 
                                             "All validation checks passed")
            await self.monitor.complete_subtask(context.operation_id, "final_validation", True)
            
            # Calculate execution time
            context.total_execution_time = (datetime.now() - context.start_time).total_seconds()
            
            await self.monitor.complete_operation(context.operation_id, True, 
            ...
            result = ProvisioningResult(
                ...
                firmware_updates_applied=len([r for r in (context.firmware_results or []) if r.success]),
                bios_settings_applied=context.bios_results.get('settings_applied', 0) 
                    if context.bios_results else 0
            )
    ...
    async def _execute_firmware_analysis(self, context: ProvisioningContext):
        ...
        try:
            ...
            for fw_type, fw_info in context.firmware_info.items():
                ...
                logger.info(f"  {fw_type.value}: {fw_info.current_version} → {fw_info.latest_version} {status} {priority}")
            
        except Exception as e:
            logger.error(f"Firmware analysis failed: {e}")
            raise WorkflowError(f"Firmware analysis failed: {e}")
    
    async def _execute_firmware_updates(self, context: ProvisioningContext, 
                                      updates_needed: List[FirmwareInfo]):
        ...
        try:
            ...
            logger.info(f"Estimated update time: {total_estimated_time} seconds ({total_estimated_time/60:.1f} minutes)")
            
            context.firmware_results = await self.firmware_manager.update_firmware_batch(
                context.device_type,
                context.target_ip,
                context.credentials['username'],
                context.credentials['password'],
            ...
            logger.info(f"Firmware updates completed: {len(successful_updates)} successful, {len(failed_updates)} failed")
            
            if failed_updates:
                logger.warning("Some firmware updates failed:")
                for result in failed_updates:
                    logger.warning(f"  ❌ {result.firmware_type.value}: {result.error_message}")
    ...
    def create_provisioning_context(self, server_id: str, device_type: str, 
                                  target_ip: str, credentials: Dict[str, str],
                                  **kwargs) -> ProvisioningContext:
        """
