...
class IPMIAutomationService:
    ...
    def detect_ipmi_vendor(self, ipmi_ip: str, username: str = 'ADMIN', password: str = 'ADMIN') -> IPMIVendor:
        """
        ...
        try:
            ...
            cmd = ['ipmitool', '-I', 'lanplus', '-H', ipmi_ip, '-U', username, '-P', password, 'mc', 'info']
            ...
            cmd = ['ipmitool', '-I', 'lanplus', '-H', ipmi_ip, '-U', username, '-P', password, 'raw', '0x30', '0x21']
    ...
    def configure_supermicro_ipmi(self, ipmi_ip: str, settings: IPMISettings) -> IPMIConfigResult:
        """
        ...
        try:
            ...
            cmd = ['ipmitool', '-I', 'lanplus', '-H', ipmi_ip, '-U', 'ADMIN', '-P', 'ADMIN', 
                   'user', 'set', 'password', '2', settings.admin_password]
            ...
            if result_cmd.returncode == 0:
                ...
                cmd = ['ipmitool', '-I', 'lanplus', '-H', ipmi_ip, '-U', 'ADMIN', '-P', settings.admin_password, 
                       'user', 'set', 'password', '2', settings.admin_password]
                result_cmd = subprocess.run(cmd, capture_output=True, text=True, timeout=10)
                if result_cmd.returncode != 0:
            ...
            cmd = ['ipmitool', '-I', 'lanplus', '-H', ipmi_ip, '-U', 'ADMIN', '-P', settings.admin_password, 'mc', 'info']
            result_cmd = subprocess.run(cmd, capture_output=True, text=True, timeout=10)
            if result_cmd.returncode == 0:
            ...
            if settings.kcs_control:
                ...
                result.warnings.append(f"KCS control setting to '{settings.kcs_control}' requires manual configuration via web interface")
            
            # Step 4: Configure Host Interface (if specified)
            if settings.host_interface:
                # This also requires vendor-specific configuration
                result.warnings.append(f"Host interface setting to '{settings.host_interface}' requires manual configuration via web interface")
            ...
            if settings.oob_license_required:
                ...
                if result_cmd.returncode == 0:
                    ...
                    result.warnings.append("OOB license may not be activated - some functions may be limited")
            
            # Step 6: Verify basic IPMI functionality
            cmd = ['ipmitool', '-I', 'lanplus', '-H', ipmi_ip, '-U', 'ADMIN', '-P', settings.admin_password, 'power', 'status']
            result_cmd = subprocess.run(cmd, capture_output=True, text=True, timeout=10)
            if result_cmd.returncode == 0:
    ...
    def configure_hp_ilo_ipmi(self, ipmi_ip: str, settings: IPMISettings) -> IPMIConfigResult:
        """
        ...
        try:
            ...
            cmd = ['ipmitool', '-I', 'lanplus', '-H', ipmi_ip, '-U', 'Administrator', '-P', 'admin', 'mc', 'info']
            result_cmd = subprocess.run(cmd, capture_output=True, text=True, timeout=10)
            ...
            result.warnings.append("IPMI/DCMI over LAN setting requires configuration via iLO web interface")
            
            # Step 3: Configure authentication settings (requires web interface)
            result.warnings.append("Host authentication and RBSU login settings require configuration via iLO web interface")
    ...
    def configure_ipmi(self, ipmi_ip: str, device_type: str, admin_password: str = None) -> IPMIConfigResult:
        """
    ...
    def _create_ipmi_settings(self, device_type: str, vendor: IPMIVendor, admin_password: str) -> IPMISettings:
    ...
    def _configure_generic_ipmi(self, ipmi_ip: str, settings: IPMISettings) -> IPMIConfigResult:
        ...
        try:
            ...
            if result_cmd.returncode == 0:
                ...
                result.warnings.append("Unknown IPMI vendor - only basic configuration applied")
            else:
    ...
    def validate_ipmi_configuration(self, ipmi_ip: str, device_type: str, admin_password: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not validation['ipmi_accessible']:
                validation['requirements_missing'].append('IPMI not accessible via ping')
                return validation
            ...
            if validation['authentication_working']:
                ...
                if validation['power_control_working']:
                    ...
                    validation['requirements_missing'].append('Power control not working')
            else:
