...
class EnhancedHardwareDetector:
    ...
    def _load_device_types(self) -> Dict[str, BMCDeviceType]:
        ...
        device_types['d1.c1.small'] = BMCDeviceType(
            ...
            hardware_spec=HardwareSpec(
                ...
                cpu_count_min=1, cpu_count_max=1,
                memory_gb_min=8, memory_gb_max=32,
        ...
        device_types['d1.c1.large'] = BMCDeviceType(
            ...
            hardware_spec=HardwareSpec(
                ...
                cpu_count_min=1, cpu_count_max=1,
                memory_gb_min=16, memory_gb_max=128,
                vendor_pattern=r'Supermicro',
                architecture='amd64'
            ),
            ipmi_settings={
        ...
        device_types['d2.c2.medium'] = BMCDeviceType(
            ...
            hardware_spec=HardwareSpec(
                ...
                cpu_count_min=1, cpu_count_max=2,
                memory_gb_min=16, memory_gb_max=64,
                vendor_pattern=r'Supermicro',
                architecture='amd64'
            ),
            ipmi_settings={
        ...
        device_types['s2.c2.large'] = BMCDeviceType(
            ...
            hardware_spec=HardwareSpec(
                ...
                cpu_count_min=2, cpu_count_max=4,
                memory_gb_min=64, memory_gb_max=512,
                vendor_pattern=r'Supermicro',
                architecture='amd64'
            ),
            ipmi_settings={
        ...
        device_types['s5.x6.c8.large'] = BMCDeviceType(
            ...
            hardware_spec=HardwareSpec(
                ...
                cpu_count_min=2, cpu_count_max=8,
                memory_gb_min=128, memory_gb_max=1024,
                vendor_pattern=r'Supermicro',
                architecture='amd64'
            ),
            ipmi_settings={
    ...
    def _extract_hardware_info(self, machine: Dict) -> Dict:
        ...
        try:
            ...
            node_info = machine.get('description', '') + ' ' + ' '.join(machine.get('tag_names', []))
            
            # Try to extract vendor
            vendor_patterns = [
                r'(Supermicro)', r'(HP[E]?)', r'(Dell)', r'(Lenovo)', r'(Intel)'
            ]
            ...
            for script_name, result in commissioning_results.items():
                if 'cpu' in script_name.lower() or 'processor' in script_name.lower():
                    ...
                    cpu_match = re.search(r'Intel.*?(?:Core|Xeon).*?(?:i[0-9]|[0-9]{4}[A-Z]?)', output, re.IGNORECASE)
                    if cpu_match:
    ...
    def _calculate_match_confidence(self, hardware_info: Dict, device_spec: BMCDeviceType) -> float:
        ...
        if spec.cpu_model_pattern and hardware_info.get('cpu_model'):
            total_criteria += 1
            if re.search(spec.cpu_model_pattern, hardware_info['cpu_model'], re.IGNORECASE):
    ...
    def validate_ipmi_configuration(self, ipmi_ip: str, username: str = 'ADMIN', 
                                  password: str = None) -> IPMIValidation:
        """
        ...
        try:
            ...
            if result.returncode != 0:
                validation.validation_errors.append(f"IPMI IP {ipmi_ip} is not reachable")
                return validation
            ...
            if password:
                ...
                validation.requires_update.append("OOB license activation status needs verification")
                
                # Check KCS control setting (vendor-specific implementation needed)
                validation.requires_update.append("KCS control setting verification needed")
                
                # Check host interface setting (vendor-specific implementation needed)
                validation.requires_update.append("Host interface setting verification needed")
    ...
    def get_available_device_types_for_machines(self, machine_ids: List[str]) -> Dict[str, List[str]]:
        """
        ...
        for machine_id in machine_ids:
            ...
            compatible_types = [device_type for device_type, confidence in matches if confidence > 0.5]
