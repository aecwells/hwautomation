...
class ConfigurationValidator:
    ...
    def validate_device_config(
        ...
        for field in required_fields:
            if field not in config:
                errors.append(
                    f"Missing required field '{field}' for device type '{device_type}'"
                )
        ...
        if "manufacturer" in config:
            ...
            if config["manufacturer"] not in valid_manufacturers:
                errors.append(
                    f"Unknown manufacturer '{config['manufacturer']}' for device type '{device_type}'"
                )
        ...
        if "motherboard" in config:
            if not isinstance(config["motherboard"], list):
                errors.append(
                    f"Motherboard field must be a list for device type '{device_type}'"
                )
            elif not config["motherboard"]:
                errors.append(
                    f"Motherboard list cannot be empty for device type '{device_type}'"
                )
        ...
        for field in boolean_fields:
            if field in config and not isinstance(config[field], bool):
                errors.append(
                    f"Field '{field}' must be boolean for device type '{device_type}'"
                )

        return errors

    def validate_template_rules(
        self, device_type: str, rules: Dict[str, Any]
    ) -> List[str]:
        ...
        for setting_name, rule in rules.items():
            if not isinstance(rule, dict):
                errors.append(
                    f"Rule '{setting_name}' for '{device_type}' must be a dictionary"
                )
                continue
            ...
            if rule["action"] not in valid_actions:
                errors.append(
                    f"Invalid action '{rule['action']}' in rule '{setting_name}' for '{device_type}'"
                )

            # Validate action-specific requirements
            if rule["action"] == "set" and "value" not in rule:
                errors.append(
                    f"Missing 'value' field for 'set' action in rule '{setting_name}' for '{device_type}'"
                )

            if rule["action"] == "conditional" and "condition" not in rule:
                errors.append(
                    f"Missing 'condition' field for 'conditional' action in rule '{setting_name}' for '{device_type}'"
                )

        return errors

    def validate_preserve_settings(
        self, device_type: str, settings: List[str]
    ) -> List[str]:
        ...
        for setting in settings:
            if not isinstance(setting, str):
                errors.append(
                    f"Preserve setting must be a string, got {type(setting)} for '{device_type}'"
                )
    ...
    def validate_setting_compatibility(
        self, setting_name: str, value: str, device_type: str
    ) -> List[str]:
    ...
    def _validate_attribute(
        self, attribute: ET.Element, component_fqdd: str
    ) -> List[str]:
        ...
        if value is None:
            errors.append(
                f"Attribute {name} in component {component_fqdd} has no value"
            )
    ...
    def _validate_dell_setting(self, setting_name: str, value: str) -> List[str]:
        ...
        if setting_name in dell_specific_settings:
            ...
            if value not in valid_values:
                errors.append(
                    f"Invalid value '{value}' for Dell setting '{setting_name}'. "
    ...
    def _validate_hpe_setting(self, setting_name: str, value: str) -> List[str]:
        ...
        if setting_name in hpe_specific_settings:
            ...
            if value not in valid_values:
                errors.append(
                    f"Invalid value '{value}' for HPE setting '{setting_name}'. "
                    f"Valid values: {valid_values}"
                )
