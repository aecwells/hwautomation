...
class ConfigurationLoader(BaseConfigLoader):
    ...
    def save_config(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            if 'device_mappings' in config:
                self._save_yaml_file(self.device_mappings_file, config['device_mappings'])
            ...
            if 'preserve_settings' in config:
                self._save_yaml_file(self.preserve_settings_file, config['preserve_settings'])
    ...
    def load_device_mappings(self) -> Dict[str, Any]:
        ...
        """
        return self._load_yaml_file(self.device_mappings_file, {})
    ...
    def _validate_device_mappings(self, mappings: Dict[str, Any]) -> List[str]:
        ...
        for device_type, config in mappings.items():
            if not isinstance(config, dict):
                errors.append(f"Device mapping for '{device_type}' must be a dictionary")
                continue
            ...
            for field in required_fields:
                if field not in config:
                    errors.append(f"Missing required field '{field}' in device mapping for '{device_type}'")
        ...
        for device_type, device_rules in rules.items():
            ...
            for setting_name, setting_rules in device_rules.items():
                if not isinstance(setting_rules, dict):
                    errors.append(f"Template rule '{setting_name}' for '{device_type}' must be a dictionary")
                    continue
