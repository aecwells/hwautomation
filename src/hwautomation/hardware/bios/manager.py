...
class BiosConfigManager(BaseBiosManager):
    ...
    def get_device_config(self, device_type: str) -> Optional[DeviceConfig]:
        ...
        return DeviceConfig(
            ...
            manufacturer=mapping.get('manufacturer', 'Unknown'),
            model=mapping.get('model', 'Unknown'),
            motherboard=mapping.get('motherboard', []),
            redfish_enabled=mapping.get('redfish_enabled', False),
            vendor_tools_available=mapping.get('vendor_tools_available', False),
            special_handling=mapping.get('special_handling', {})
        )

    def select_optimal_method(self, device_type: str, target_ip: str) -> MethodSelectionResult:
    ...
    def apply_bios_config_smart(
        ...
        """
        logger.info(f"Starting smart BIOS config for {device_type} at {target_ip} (dry_run={dry_run})")
        
        try:
            # Step 1: Select optimal method
            method_result = self.select_optimal_method(device_type, target_ip)
            logger.info(f"Selected method: {method_result.recommended_method.value} "
            ...
            backup_file = None
            if backup_enabled:
                backup_file = self._create_backup(current_config, device_type, target_ip)
            ...
            if dry_run:
                ...
                return BiosConfigResult(
                    success=True,
                    method_used=method_result.recommended_method,
                    settings_applied={"dry_run": "Configuration validated successfully"},
                    settings_failed={},
                    backup_file=backup_file,
                    validation_errors=[],
            ...
            push_success = self.push_config(modified_config, target_ip, username, password)
    ...
    def _create_backup(self, config: ET.Element, device_type: str, target_ip: str) -> str:
