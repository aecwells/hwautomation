...
class RedfishConfigParser(BaseConfigParser):
    ...
    def _extract_bios_attributes(self, json_data: Dict[str, Any]) -> Dict[str, str]:
        ...
        for key, value in json_data.items():
            if key not in ['Model', 'ServiceTag', 'Bios', 'Oem', 'Attributes'] and isinstance(value, (str, int, bool)):
    ...
    def create_redfish_patch_payload(self, settings: Dict[str, str]) -> str:
        ...
        try:
            ...
            self.logger.debug(f"Created Redfish PATCH payload with {len(settings)} settings")
            return json_str
