...
class ParserFactory:
    ...
    def register_parser(
        self, format_name: str, parser_class: Type[BaseConfigParser]
    ) -> None:
        ...
        logger.debug(
            f"Registered parser {parser_class.__name__} for format: {format_name}"
        )
    ...
    def get_parser(self, format_name: str) -> Optional[BaseConfigParser]:
        ...
        try:
            parser = parser_class()
            logger.debug(
                f"Created parser {parser_class.__name__} for format: {format_name}"
            )
            return parser

        except Exception as e:
            logger.error(
                f"Failed to create parser {parser_class.__name__} for {format_name}: {e}"
            )
            return None
