...
class DeviceHandlerFactory:
    ...
    def register_handler(
        self, manufacturer: str, handler_class: Type[BaseDeviceHandler]
    ) -> None:
    ...
    def get_handler(
        self, device_type: str, device_config: DeviceConfig
        ...
        try:
            ...
            if not handler.can_handle(device_type):
                logger.warning(
                    f"Handler {handler_class.__name__} cannot handle device type: {device_type}"
                )
                return None

            logger.debug(
                f"Created handler {handler_class.__name__} for device type: {device_type}"
            )
            return handler

        except Exception as e:
            logger.error(
                f"Failed to create handler {handler_class.__name__} for {device_type}: {e}"
            )
            return None
