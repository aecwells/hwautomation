...
def run_command(
    ...
    try:
        ...
        print_colored(
            f"💡 Try installing it with: pip install {cmd[0]}", Colors.WARNING
        )
...
def lint_code(paths: List[str]) -> int:
    ...
    cmd = ["flake8"] + paths
    returncode, stdout, stderr = run_command(
        cmd, "Running Flake8 linting", check=False, capture_output=True
    )
...
def type_check(paths: List[str]) -> int:
    ...
    cmd = ["mypy"] + paths
    returncode, stdout, stderr = run_command(
        cmd, "Running MyPy type checking", check=False, capture_output=True
    )
...
def security_scan(paths: List[str]) -> int:
    ...
    cmd = ["bandit", "-r"] + paths
    returncode, stdout, stderr = run_command(
        cmd, "Running Bandit security scan", check=False, capture_output=True
    )
...
def run_tests(test_type: str = "unit") -> int:
    ...
    returncode, stdout, stderr = run_command(
        cmd, f"Running {test_type} tests", check=False
    )
...
def generate_coverage_report() -> int:
    ...
    returncode, stdout, stderr = run_command(
        cmd, "Generating coverage report", check=False
    )

    if returncode == 0:
        print_colored(
            "📊 Coverage report generated in htmlcov/index.html", Colors.OKGREEN
        )
...
Examples:
  ...
  python tools/quality/code_quality.py --coverage
    ...
    # Operation flags
    parser.add_argument(
        "--format", action="store_true", help="Run code formatting (black, isort)"
    )
    parser.add_argument("--lint", action="store_true", help="Run code linting (flake8)")
    parser.add_argument(
        "--type-check", action="store_true", help="Run type checking (mypy)"
    )
    parser.add_argument(
        "--security", action="store_true", help="Run security scanning (bandit)"
    )
    parser.add_argument(
        "--test",
