...
class WorkflowMonitor:
    ...
    def get_workflow_status(self, workflow_id):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/orchestration/workflow/{workflow_id}/status"
            )
    ...
    def get_workflow_debug(self, workflow_id):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/orchestration/workflow/{workflow_id}/debug"
            )
            if response.status_code == 200:
                return response.json()
            else:
    ...
    def monitor_workflow(self, workflow_id, interval=5):
        ...
        try:
            while True:
                ...
                print(
                    f"[{timestamp}] Status: {workflow_status} | Current Step: {current_step}"
                )
                ...
                if status.get("steps"):
                    for i, step in enumerate(status["steps"]):
                        ...
                        indicator = (
                            "▶"
                            if i == current_step
                            else ("✓" if step_status == "completed" else "○")
                        )
                        print(
    ...
    def show_workflow_debug(self, workflow_id):
        ...
        if debug_info.get("step_details"):
            ...
            for step in debug_info["step_details"]:
                indicator = (
                    "▶"
                    if step["index"] == debug_info.get("current_step")
                    else ("✓" if step["status"] == "completed" else "○")
                )
                print(
                    f"  {indicator} Step {step['index'] + 1}: {step['name']} ({step['status']})"
                )
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print(
            "  python workflow_monitor.py list                    - List all workflows"
        )
        print(
            "  python workflow_monitor.py status <workflow_id>    - Get workflow status"
        )
    ...
    if command == "list":
        ...
        if "error" in workflows:
            ...
            for wf in workflows.get("workflows", []):
                print(
                    f"  - {wf['id']}: {wf.get('status', 'unknown')} ({wf.get('type', 'unknown')})"
                )
