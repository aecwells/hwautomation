    ...
    try:
        while True:
            try:
                ...
                response = requests.get("http://localhost:5000/api/orchestration/workflows")
                if response.status_code == 200:
                    data = response.json()
                    current_workflows = set(wf['id'] for wf in data.get('workflows', []))
                    ...
                    for workflow in data.get('workflows', []):
                        ...
                        # Get detailed status
                        status_response = requests.get(f"http://localhost:5000/api/orchestration/workflow/{workflow_id}/status")
                        if status_response.status_code == 200:
                            detailed_status = status_response.json()
                            current_step_index = detailed_status.get('current_step_index')
                            current_step_name = detailed_status.get('current_step_name', 'unknown')
                            
                            # Display current step more clearly
                            if current_step_index is not None:
                                step_display = f"{current_step_index + 1}: {current_step_name}"
                            else:
                            ...
                            if detailed_status.get('steps'):
                                for i, step in enumerate(detailed_status['steps']):
                                    ...
                                    if step.get('result') and len(str(step['result'])) < 100:
