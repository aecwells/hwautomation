...
def cmd_list_templates(args):
    ...
    if not templates:
        ...
        print("  Use 'generate' command to create templates from device configurations.")
...
def cmd_pull_config(args):
    ...
    try:
        current_config = manager.pull_current_bios_config(args.target_ip, args.username, args.password)
...
Examples:
  ...
  %(prog)s apply-config s2_c2_small 192.168.1.100  # Apply config to system
    ...
    list_templates_parser = subparsers.add_parser(
        'list-templates', 
        help='List available XML templates'
    )
    
    # Show config command
    show_parser = subparsers.add_parser(
        'show-config', 
        help='Show configuration for device type'
    )
    ...
    generate_parser = subparsers.add_parser(
        'generate-xml', 
        help='Generate XML configuration'
    )
    ...
    save_parser = subparsers.add_parser(
        'save-template', 
        help='Save XML template for device type'
    )
    ...
    pull_parser = subparsers.add_parser(
        'pull-config',
        help='Pull current BIOS configuration from target system'
    )
    ...
    apply_parser.add_argument('--dry-run', action='store_true', help='Show what would be changed without applying')
