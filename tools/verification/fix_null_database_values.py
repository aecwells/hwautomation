...
def fix_null_values():
    ...
    try:
        ...
        for server_id, device_type, workflow_id, commissioning_status, status_name in null_servers:
            logger.info(f"Processing server: {server_id}")
            logger.info(f"  Current values - device_type: {device_type}, workflow_id: {workflow_id}, commissioning_status: {commissioning_status}, status_name: {status_name}")
            ...
            try:
                ...
                memory_gb = hardware_info.get('memory', 0) / (1024**3) if hardware_info.get('memory') else 0
                ...
                logger.info(f"  Inferred values - device_type: {inferred_device_type}, workflow_id: {inferred_workflow_id}, commissioning_status: {inferred_commissioning_status}")
                
                # Update NULL values only
                updates_made = []
                
                if device_type is None:
                    db_helper.updateserverinfo(server_id, 'device_type', inferred_device_type)
                    updates_made.append(f"device_type -> {inferred_device_type}")
                
                if workflow_id is None:
                    db_helper.updateserverinfo(server_id, 'workflow_id', inferred_workflow_id)
                    updates_made.append(f"workflow_id -> {inferred_workflow_id}")
                
                if commissioning_status is None:
                    db_helper.updateserverinfo(server_id, 'commissioning_status', inferred_commissioning_status)
                    updates_made.append(f"commissioning_status -> {inferred_commissioning_status}")
        ...
        cursor.execute("SELECT COUNT(*) FROM servers WHERE commissioning_status IS NULL")
        null_commissioning_status = cursor.fetchone()[0]
        
        logger.info(f"Remaining NULL values: device_type={null_device_type}, workflow_id={null_workflow_id}, commissioning_status={null_commissioning_status}")
