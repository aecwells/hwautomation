...
def check_git_repo() -> bool:
    ...
    if not Path('.git').exists():
        ...
        print_colored("💡 Run 'git init' to initialize a git repository", Colors.WARNING)
        return False
...
def setup_virtual_environment(venv_name: str = "hwautomation-env") -> bool:
    ...
    if venv_path.exists():
        print_colored(f"📁 Virtual environment '{venv_name}' already exists", Colors.WARNING)
        response = input("Do you want to recreate it? (y/N): ").lower().strip()
        if response in ['y', 'yes']:
            print_colored(f"🗑️  Removing existing virtual environment...", Colors.WARNING)
...
def install_dev_dependencies(use_venv: bool = True) -> bool:
    ...
    if not run_command([python_exe, '-m', 'pip', 'install', '-e', '.[dev]'], 
...
def setup_pre_commit_hooks() -> bool:
    ...
    if not run_command(['pre-commit', 'install'], "Installing pre-commit hooks"):
        print_colored("💡 Make sure pre-commit is installed: pip install pre-commit", Colors.WARNING)
        return False
    ...
    if result:
        ...
        print_colored("⚠️  Pre-commit found issues (this is normal for first run)", Colors.WARNING)
        print_colored("💡 Run 'pre-commit run --all-files' again to see if issues are fixed", Colors.WARNING)
...
def print_next_steps():
    ...
    print_colored("   git commit -m 'feat: setup development environment'", Colors.WARNING)
