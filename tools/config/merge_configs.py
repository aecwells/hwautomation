...
def merge_device_mappings():
    ...
    shutil.copy2(
        "configs/bios/device_mappings.yaml", "configs/bios/device_mappings.yaml.backup"
    )
    ...
    for server_id, config in generated["device_types"].items():
        ...
        if (
            server_id.startswith(("s1.", "s2.", "s3.", "s4.", "s5."))
            or vendor == "supermicro"
            or server_id in ["d1.c1.small", "d1.c1.medium", "d1.c1.large"]
        ):
...
def update_orchestration_dropdown(device_types):
    ...
    for series in sorted(series_groups.keys()):
        if series != "legacy":
            options.append(
                f'                        <optgroup label="{series.upper()} Series">'
            )

        for device_type in sorted(series_groups[series]):
            options.append(
                f'                        <option value="{device_type}">{device_type}</option>'
            )
