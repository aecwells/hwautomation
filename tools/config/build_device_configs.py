...
def main():
    ...
    for device_type in sample_types:
        ...
        specs = config.get("hardware_specs", {})
        print(
            f"  • {device_type}: {specs.get('cpu_cores', 0)} cores, {specs.get('ram_gb', 0)}GB RAM"
        )
    ...
    print(
        "2. Merge configs/bios/device_mappings_generated.yaml with your existing device_mappings.yaml"
    )
