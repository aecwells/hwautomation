...
from test_framework import (
    PerformanceTestMixin, MockDataFactory, DatabaseTestHelper, 
    AsyncTestHelper, TestAssertions
)
...
def mock_firmware_manager():
    ...
    firmware_info = {
        "bios": {"current_version": "1.2.3", "latest_version": "1.2.4", "update_required": True},
        "bmc": {"current_version": "4.5.6", "latest_version": "4.5.6", "update_required": False}
    }
...
def sample_test_data():
    ...
    return {
        ...
        "workflows": [
            MockDataFactory.create_workflow_data(f"workflow-{i:03d}")
            for i in range(2) 
        ],
        "maas_machines": [
            MockDataFactory.create_maas_machine_data(f"node-{i:03d}")
...
def pytest_runtest_setup(item):
    ...
    if item.get_closest_marker("integration") and not os.getenv("RUN_INTEGRATION_TESTS"):
