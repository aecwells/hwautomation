...
class TestProgressCallback(ProgressCallback):
    ...
    async def on_progress_event(self, event):
        ...
        print(
            f"📊 [{event.timestamp.strftime('%H:%M:%S')}] {event.event_type.value}: {event.message}"
        )
...
async def test_phase3_monitoring():
    ...
    try:
        ...
        if validation:
            ...
            print(
                f"      Pre-flight: {'✅ Success' if pre_flight.get('success') else '❌ Failed'}"
            )
            post_config = validation.get("post_configuration", {})
            print(
                f"      Post-configuration: {'✅ Success' if post_config.get('success') else '❌ Failed'}"
            )
    ...
    try:
        ...
        if monitor:
            ...
            await monitor.start_subtask(
                operation_id, "connectivity_check", "Testing connectivity"
            )
            await asyncio.sleep(0.1)  # Simulate work
            await monitor.complete_subtask(
                operation_id, "connectivity_check", True, "Connectivity OK"
            )

            await monitor.start_subtask(
                operation_id, "capability_validation", "Validating capabilities"
            )
            await asyncio.sleep(0.1)  # Simulate work
            await monitor.complete_subtask(
                operation_id, "capability_validation", True, "Capabilities validated"
            )

            await monitor.start_subtask(
                operation_id, "configuration_apply", "Applying configuration"
            )
            await asyncio.sleep(0.1)  # Simulate work
            await monitor.complete_subtask(
                operation_id, "configuration_apply", True, "Configuration applied"
            )

            # Complete operation
            await monitor.complete_operation(
                operation_id, True, "Test operation completed successfully"
            )
        ...
        if monitor:
            ...
            await monitor.complete_operation(
                test_op_id, True, "Callback test completed"
            )
        ...
        if monitor:
            ...
            await monitor.log_error(
                ...
                {"error_type": "connectivity_timeout", "retry_attempted": True},
            )

            await monitor.log_warning(error_op_id, "Performance degradation detected")
            await monitor.complete_operation(
                error_op_id, False, "Operation failed with errors"
            )

            error_status = monitor.get_operation_status(error_op_id)
            if error_status:
                print(
                    f"   ✅ Error tracking: {error_status.error_count} errors, {error_status.warning_count} warnings"
                )
        else:
