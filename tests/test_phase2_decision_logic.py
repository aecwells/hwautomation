...
def test_phase2_decision_logic():
    ...
    try:
        ...
        print(f"      Vendor tool settings: {len(reliability_analysis.vendor_settings)}")
        ...
        print(f"      Redfish total time: {perf_estimate.get('redfish_total_time', 0):.1f}s")
        print(f"      Vendor tool total time: {perf_estimate.get('vendor_tool_total_time', 0):.1f}s")
        print(f"      Estimated total time: {perf_estimate.get('estimated_total_time', 0):.1f}s (parallel)")
        print(f"      Redfish batch count: {perf_estimate.get('redfish_batch_count', 0)}")
        print(f"      Vendor setting count: {perf_estimate.get('vendor_setting_count', 0)}")
        ...
        for i, batch in enumerate(perf_analysis.batch_groups):
            ...
            print(f"      Batch {i+1}: {method} - {settings_count} settings - {estimated_time:.1f}s")
        ...
        if phase2_result.get('success'):
            ...
            print(f"      Estimated time: {summary.get('estimated_total_time', 0):.1f}s")
            
            # Show method analysis
            analysis = phase2_result.get('method_analysis', {})
            print(f"\n   📋 Method analysis details:")
            print(f"      Redfish settings: {list(analysis.get('redfish_settings', {}).keys())}")
            print(f"      Vendor settings: {list(analysis.get('vendor_settings', {}).keys())}")
            
        else:
            print(f"   ❌ Phase 2 dry run failed: {phase2_result.get('error', 'Unknown error')}")
            return False
