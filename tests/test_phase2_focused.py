...
try:
    from hwautomation.hardware.bios_decision_logic import BiosSettingMethodSelector, SettingPriority, ConfigMethod
except ImportError as e:
    print(f"Import error: {e}")
    print("Testing decision logic module in isolation...")
    
    # Copy the decision logic classes locally for testing
    exec(open(Path(__file__).parent.parent / "src" / "hwautomation" / "hardware" / "bios_decision_logic.py").read())
...
def test_decision_logic_focused():
    ...
    try:
        ...
        print(f"\n   📈 Performance-optimized analysis:")
        perf_analysis = method_selector.analyze_settings(sample_settings, prefer_performance=True)
        ...
        reliability_analysis = method_selector.analyze_settings(sample_settings, prefer_performance=False)
        
        print(f"      Redfish: {len(reliability_analysis.redfish_settings)} settings")
        print(f"      Vendor tools: {len(reliability_analysis.vendor_settings)} settings")
        ...
        print(f"      Performance-optimized: {perf_estimate.get('estimated_total_time', 0):.1f}s total")
        print(f"         - Redfish: {perf_estimate.get('redfish_total_time', 0):.1f}s")
        print(f"         - Vendor: {perf_estimate.get('vendor_tool_total_time', 0):.1f}s")
        print(f"      Reliability-optimized: {rel_estimate.get('estimated_total_time', 0):.1f}s total")
        print(f"         - Redfish: {rel_estimate.get('redfish_total_time', 0):.1f}s")
        print(f"         - Vendor: {rel_estimate.get('vendor_tool_total_time', 0):.1f}s")
        ...
        mock_available_settings = {
            "BootMode", "SecureBoot", "PowerProfile", "QuietBoot",
            "IntelHyperThreadingTech", "ProcessorEistEnable",
            "SomeOtherSetting"
        }
        
        validation_results = method_selector.validate_redfish_capabilities(mock_available_settings)
        
        available = [setting for setting, available in validation_results.items() if available]
        unavailable = [setting for setting, available in validation_results.items() if not available]
        ...
        if 'redfish_preferred' in bios_setting_methods:
            ...
            print(f"      Sample Redfish preferred settings: {list(preferred.keys())[:3]}")
