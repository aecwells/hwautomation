...
from hwautomation.utils.env_config import Config, load_config, ConfigError
...
class TestConfigValidation:
    ...
    def test_missing_maas_credentials_warning(self, caplog):
        ...
        try:
            ...
            config = Config()
            assert 'MaaS configuration provided but missing authentication' in caplog.text
        finally:
            # Restore environment
            os.environ.clear()
...
""")
        ...
        env_vars_to_clear = [
            'PROJECT_NAME', 'DEBUG', 'DATABASE_PATH', 'DATABASE_TABLE_NAME',
            'MAAS_HOST', 'MAAS_CONSUMER_KEY', 'MAAS_TOKEN_KEY', 'MAAS_TOKEN_SECRET',
            'MAAS_VERIFY_SSL', 'IPMI_USERNAME', 'IPMI_PASSWORD', 'SSH_USERNAME', 'LOG_LEVEL'
        ]
